...
#define EEPROM_CHECK1       (0x0016)                        // EEPROM校验字1
...
// <-Industry modify by libo 2014/12/09 18:8:9
// 常用功能码参数表：类型、索引、最优值
typedef struct FUNCCODE_INDUSTRY_STRUCT
{
    Uint16                  funcGroup;                  // 功能码组
    Uint16                  funcGrade;                  // 功能码组序号
    Uint16                  funcodeIndex;               // 功能码索引
    Uint16                  bestInit;                   // 最优值
} FUNCCODE_INDUSTRY;

extern const FUNCCODE_INDUSTRY funcCodeIndustry[];
...
struct FC_GROUP_HIDE_STRUCT
{
    ...
    Uint16 a;           // E组隐藏特性(A0-AF)
    Uint16 b;           // B组隐藏特性(B0-BF)
    Uint16 c;           // Y组隐藏特性(C0-Cf)
};
union FC_GROUP_HIDE
{
    Uint16 all[5];
...
struct ERROR_SCENE_STRUCT
{
    ...
    Uint16 errorCoeff;                  // 第三次(最近一次)故障时反电动势
...
struct PG_PARA_STRUCT
{
    ...
    Uint16 enCoderPole;             // F1-34  旋变极对 数
...
struct VC_PARA
{
    ...
    Uint16 weakFlusMode;            //  F2-18同步机弱磁模式
    Uint16 weakFlusCoef;            //  F2-19同步机弱磁系数
    Uint16 weakFlusCurMax;          //  F2-20最大弱磁电流
    Uint16 weakFlusAutoCoef;        //  F2-21弱磁自动调谐系数
    Uint16 weakFlusIntegrMul;       //  F2-22弱磁积分倍数

};
//------------------------------------------------

struct SYNCHRO_MOtOR_VC_PARA  // 第1电机为同步机时控制参数(f2-23~f2-49) stamp_PI500E_061
{
    Uint16 pmsmWeakCoef;            //  f2-23 弱磁深度
    Uint16 pmsmWeakDeep;            //  f2-24 初始位置检测电流
    Uint16 startPositionCheckAble;  //  F2-25 初始位置是否检测
    Uint16 speedHoopMode;           //  F2-26 速度环模式选择
    Uint16 mostOutpowerCoef;        //  F2-27 最大出力调整系数

    Uint16 udcLimitFrqEnable;       //  F2-28 跟据母线电压对频率限幅使能
    Uint16 PrecedentCompMode;       //  F2-29 前馈补偿模式
    Uint16 PmParEstIsKpK;           //  f2-30 调谐时电流环KP
    Uint16 PmParEstIsKiK;           //  F2-31 调谐时电流环KI
    Uint16 IPMPosZSigable;          //  F2-32 Z信号校正使能

    Uint16 PmsmSvcSpeedLpfTs;       //  F2-33 同步机SVC速度滤波级别
    Uint16 PmsmSvcKb;               //  F2-34 同步机SVC速度估算比例增益
    Uint16 PmsmSvcKa;               //  f2-35 同步机SVC速度估算积分增益
    Uint16 PmsmSvcIdMaxLowSpeed;    //  F2-36 同步机SVC初始励磁电流限幅
    Uint16 PmsmSvcFcLow;            //  F2-37 同步机SVC最低载波频率

    Uint16 lowFrqRunMode;           //  f2-38 低频运行方式
    Uint16 lowFrqDealAble;          //  F2-39 低频生效
    Uint16 lowChangeUpDown;         //  f2-40 低频频率步长
    Uint16 lowBreakCurrut;          //  f2-41 低频制动电流
    Uint16 PmsmSvcspeedTrack;       //  F2-42 同步机SVC速度跟踪
    Uint16 zeroServoEnable;         //  F2-43 零伺服使能
    Uint16 switchFrq;               //  F2-44 切换频率
    Uint16 zeroServoKp;             //  F2-45 零伺服速度环比例增益
    Uint16 zeroServoKi;             //  F2-46 零伺服速度环 积分时间
    Uint16 stopForbidReverse;       //  F2-47 停机禁止反转
    Uint16 stopAngle;               //  F2-48 停机角度
    Uint16 onlineTuneEnable;        //  F2-49 在线调谐使能 0:关闭 1:上电第一次运行前调谐 2:运行前调谐
    Uint16 onlineCoeffEnable;       //  F2-50 在线反电动势辨识 0:关闭 1:开启
    Uint16 rateFrqPercent;          //  F2-51 额定频率百分比
};
...
struct MOTOR_FC
{
    union MOTOR_PARA motorPara;     //  Ax-26 Ax-00  Ax-26 第2/3/4电机参数. 同第1电机参数
    union PG_PARA    pgPara;        //  Ax-36 Ax-27  Ax-36第1电机PG卡参数
    ...
    Uint16 FluxWeakDepth;           //  输出电压饱和裕量             66
    Uint16 PmInitPosDetCur;         //  同步电机初始位置辨识电流初始值 67

    Uint16 PmsmIniPosDetEn;         // 同步电机初始位置检测模式    68
    Uint16 SpeedLoopMode;           // 速度环模式                     69
    Uint16 SalientRateCoff;         // 凸极率增益系数               70
    Uint16 PmsmMaxTorqCtrlEnable;   // 最大转矩比电流使能          71
    Uint16 CloseLoopVolDCp;         // 闭环电流环解耦模式           72

    Uint16 PmTuneIsKp;              // 同步电机调谐时电流环KP        73
    Uint16 PmTuneIsKi;              // 同步电机调谐时电流环Ki        74
    Uint16 ZSigEnable;              // Z信号校正使能           75
    Uint16 SvcSpeedLpfTs;           // SVC速度滤波系数            76
    Uint16 SVCFluxKb;               // 磁通系数 Kp               77

    Uint16 SVCFluxKa;               // 磁通系数 Ki                 78
    Uint16 SvcIdMaxForLowSpeed;     // 低速励磁电流最大值           79
    Uint16 SvcWaveForLowSpeed;      // SVc低速载波频率          80
    Uint16 SvcLowSpeedMode;         // SVC低速模式              81
    Uint16 BrakeFreq;               // 制动频率 82

    Uint16 Step;                   //步长      83
    Uint16 ImBrake;                //          84
    Uint16 SpeedCheckEn;           // 转速追踪使能 85
    Uint16 ZeroServerEn;           // 零伺服使能  86
    Uint16 SwitchFreForZeroServer; // 切换频率  87

    Uint16 KpForZeroServer;        //88
    Uint16 KiForZeroServer;        //89
    Uint16 AntiDirEnable;          // 防反转功能使能               90
    Uint16 PosSetAdd;              // 防反转时停机角度             91
};
//------------------------------------------------/
...
// 其中,16-63   -   掉电记忆使用
#define EEPROM_INDEX_USE_LENGTH     929+1     //stamp_zhd 最后一个eeprom地址+1
...
#define FUNCCODE_GROUP_F0   00      // F0组
#define FUNCCODE_GROUP_F1   01      // F1组
#define FUNCCODE_GROUP_F2   02      // F2组
#define FUNCCODE_GROUP_F3   03      // F3组
#define FUNCCODE_GROUP_F4   04      // F4组
#define FUNCCODE_GROUP_F5   05      // F5组
#define FUNCCODE_GROUP_F6   06      // F6组
#define FUNCCODE_GROUP_F7   07      // F7组
#define FUNCCODE_GROUP_F8   08      // F8组
#define FUNCCODE_GROUP_F9   09      // F9组
#define FUNCCODE_GROUP_FA   10      // FA组
#define FUNCCODE_GROUP_FB   11      // FB组
#define FUNCCODE_GROUP_FC   12      // FC组
#define FUNCCODE_GROUP_FD   13      // FD组
...
#define FUNCCODE_GROUP_A1   18      //A1
#define FUNCCODE_GROUP_A2   19      // A2
#define FUNCCODE_GROUP_A3   20      //  A3
#define FUNCCODE_GROUP_A4   21      //  A4
#define FUNCCODE_GROUP_A5   22      //  A5
#define FUNCCODE_GROUP_A6   23      //  A6
#define FUNCCODE_GROUP_A7   24      // A7
#define FUNCCODE_GROUP_A8   25      //  A8
#define FUNCCODE_GROUP_A9   26      // A9
#define FUNCCODE_GROUP_AA   27      // AA
#define FUNCCODE_GROUP_AB   28      //  AB
#define FUNCCODE_GROUP_AC   29      //  AC
#define FUNCCODE_GROUP_AD   30      // AD
#define FUNCCODE_GROUP_AE   31      // AE
                                                                               
#define FUNCCODE_GROUP_AF   32      // b0组
#define FUNCCODE_GROUP_B0   33      // b1组
#define FUNCCODE_GROUP_B1   34      // b2组
#define FUNCCODE_GROUP_B2   35      // b3组
#define FUNCCODE_GROUP_B3   36      // b4组
#define FUNCCODE_GROUP_B4   37      // b5组
#define FUNCCODE_GROUP_B5   38      // b6组
#define FUNCCODE_GROUP_B6   39      // b7组
#define FUNCCODE_GROUP_B7   40      // b8组
#define FUNCCODE_GROUP_B8   41      // b9组
#define FUNCCODE_GROUP_B9   42      // bA组
#define FUNCCODE_GROUP_BA   43      // bB组
#define FUNCCODE_GROUP_BB   44      // bC组
#define FUNCCODE_GROUP_BC   45      // bD组
#define FUNCCODE_GROUP_BD   46      // bE组
#define FUNCCODE_GROUP_BE   47      // bF组
                                                                               
#define FUNCCODE_GROUP_BF   48      // bf组
#define FUNCCODE_GROUP_C0   49      // 组
#define FUNCCODE_GROUP_C1   50      // 组
#define FUNCCODE_GROUP_C2   51      // 组
#define FUNCCODE_GROUP_C3   52      // 组
#define FUNCCODE_GROUP_C4   53      // 组
#define FUNCCODE_GROUP_C5   54      // 组
#define FUNCCODE_GROUP_C6   55      // 组
                                                                               
#define FUNCCODE_GROUP_C7   56      // 组
#define FUNCCODE_GROUP_C8   57      // 组
#define FUNCCODE_GROUP_C9   58      // 组
#define FUNCCODE_GROUP_CA   59      // 组
#define FUNCCODE_GROUP_CB   60      // 组
#define FUNCCODE_GROUP_CC   61      // 组
#define FUNCCODE_GROUP_CD   62      // 组
#define FUNCCODE_GROUP_CE   63      // 组
#define FUNCCODE_GROUP_CF   64      // 组
...
#define F2NUM           (23+29  )   // F2  矢量控制参数
#define F3NUM           (34+ 3  )   // F3  V/F控制参数
#define F4NUM           (41+ 0  )   // F4  输入端子
#define F5NUM           (24+ 0  )   // F5  输出端子
#define F6NUM           (16+ 11  )   // F6  启停控制
#define F7NUM           (15+ 2  )   // F7  键盘与显示

#define F8NUM           (57+ 0  )   // F8  辅助功能
#define F9NUM           (71+ 10  )  // F9  故障与保护
#define FANUM           (29+ 0  )   // FA  PID功能
#define FBNUM           (10+ 0  )   // FB  摆频、定长和计数

#define FCNUM           (52+ 0  )   // FC  多端速、PLC
#define FDNUM           (10+ 0  )   // FD  通讯参数
...
#define A2NUM           (F1NUM+2+F2NUM) // A2 F1NUM+2+F2NUM = F1NUM+5+F2NUM -3
#define A3NUM           ( 0+1   )   // A3
#define A4NUM           ( 0+1   )   // A4
#define A5NUM           (10+ 0  )   // A5
#define A6NUM           (30+0   )   // A6
#define A7NUM           (9+3    )   // A7
#define A8NUM           (12+0    )   // A8
#define A9NUM           (1     )   // A9
#define AANUM           (1+ 0  )    // AA
#define ABNUM           (1+ 0   )   // AB
#define ACNUM           (28     )   // AC
...
#define U0NUM     (100+0-4-10)   // U0  显示使用，不占用EEPROM，尽量少的占用程序空间(无出厂值、上下限，但有属性)
#define U1NUM     ( 0+1)    // U1
#define U2NUM     ( 0+1)    // U2
#define U3NUM     (10+2+4+10)    // U3
...
struct FUNCCODE_CODE 
{
    ...
    struct SYNCHRO_MOtOR_VC_PARA vcparaM1;// 第1电机为同步机时控制参数
    ...
    Uint16 VFOvShockMode;           // F3-12  振荡抑制增益模式
    Uint16 vfVoltageSrc;            //  F3-13  VF分离的电压源
    Uint16 vfVoltageDigtalSet;      //  F3-14  VF分离的电压源数字设定
    Uint16 vfVoltageAccTime;        //  F3-15  VF分离的电压上升时间
    Uint16 vfVoltageDecTime;        //   F3-16  VF分离的电压减速时间
    Uint16 vfSeparateStopMode;      //   F3-17  VF分离停机方式选择
    Uint16 VFOverCurLimitPoint ;    //  F3-18   VF过流失速动作电流
    Uint16 VFOverCurEnableSwitch;   //  F3-19   VF过流失速使能
    Uint16 VFOverCurKp;             //  F3-20   VF过流失速抑制增益

    Uint16 VFOverCurHighFreqComCoff;//  F3-21   VF倍速过流失速动作电流补偿系数
    Uint16 VFOverVdcLimitPoint;     //  F3-22   VF过压失速动作电压
    Uint16 VFOverVdcEnableSwitch;   //  F3-23   VF过压失速使能
    Uint16 VFOverVdcFreqKp;         //  F3-24   VF过压失速抑制频率增益
    Uint16 VFOverVdcVoltKp;         //  F3-25   VF过压失速抑制电压增益

    Uint16 VFOverVdcLimitFreq;              // F3-26  过压失速最大上升限制频率
    Uint16 VFWsComTimeCoff;                 //  F3-27  转差补偿时间常数
    Uint16 VFAutoUpFreqEnableSwitch;        //  F3-28  自动升频使能

    Uint16 VFMinDriveCur;                   //  F3-29   最小电动力矩电流
    Uint16 VFMaxFdbCur;                     //  F3-30   最大发电力矩电流
    Uint16 VFAutoUpFreqKp;                  //  F3-31   自动升频KP
    Uint16 VFAutoUpFreqKi;                  //  F3-32   自动升频KI
    Uint16 torquepCompGain;                 //  F3-33   在线转矩补偿增益


    Uint16 rsvdF3[3];                     // F4-34~36 VF参数组   MD500
    //======================================
    //======================================
    // F4 输入端子
    ...
    Uint16 curveSet2P3[4];                  // F4-23,...,F1-26  曲线3，2点
    ...
    Uint16 diLogic[2];                      // F4-35  DI有效状态选择1
                                            // F4-36  DI有效状态选择2
    Uint16 diDelayTime[3];                  // F4-37  DI1延迟时间
                                            // F4-38  DI1延迟时间
                                            // F4-39  DI1延迟时间
    Uint16 ai2ModeSel;                     // F4-40  AI2模式选择
    ...
    Uint16 ao1ModeSel;                      // F5-23  AO模式选择
    ...
    Uint16 breakDutyOutTime;        // F6-16  制动电阻开通时间
    Uint16 pdSearchCurKp ;          // F6-17  转速跟踪闭环电流KP
    Uint16 SpdSearchCurKi ;         // F6-18  转速跟踪闭环电流KI
    Uint16 SpdSearchCur;            // F6-19  转速跟踪闭环电流大小
    Uint16 SpdSearchScanCurDownLimit;//F6-20  转速跟踪闭环电流下限定值
    Uint16 SpdSearchVoltUpTime;     // F6-21  转速跟踪电压上升时间
    Uint16 SpdSearchOffMagTime;     // F6-22  去磁时间
    Uint16 rsvdF6[4];               //23-26

    // F7 键盘与显示
    Uint16 checkDigitalTube;        // F7-00  数码管缺画检验
    ...
    Uint16 productVersion;          // F7-08  整流桥软件版本号productVersion
    Uint16 runTimeAddup;            // F7-09  累计运行时间, 单位: h

    Uint16 coreSoftVersion;         // F7-10  产品号coreSoftVersion
    ...
    Uint16 CoreTempSoftversion;     //15,CoreTempSoftversion
    Uint16 tempsoftversion;         // 16,tempsoftversion
    ...
    Uint16 outPowerGain;            // F8-54  输出功率系数
    Uint16 outCurrentGain;          // F8-55  输出电流系数
    Uint16 outVolMode;              // F8-56  输出电压显示方式
    //======================================
    // F9 故障与保护
    ...
    Uint16 brakeStartVoltage;           // F9-08  制动起始电压
    #elif DEBUG_MD290_SEARIS
    Uint16 brakeStartVoltage;           // F9-08  制动起始电压
    #else
    Uint16 brakeStartVoltage;           // F9-08  制动起始电压

    //Uint16 LowVolErrorAutoRst;        // F9-08  欠压故障自动复位
    #endif
    ...
    union ERROR_SCENE errorScene3;      // F9-17  第三次(最近一次)故障时频率
                                        ...
                                        // F9-25  第三次(最近一次)故障时的反电动势
    Uint16 rsvdF92;                     //  F9-26

    union ERROR_SCENE errorScene2;      // F9-27  第二次故障现场
    Uint16 rsvdF921;                    // F9-36

    union ERROR_SCENE errorScene1;      // F9-37  第一次故障殖?
    Uint16 rsvdF922;                    //  F9-46

    Uint16 errorAction[5];              // F9-47  -F9-51  故障时保护动作选?-5
    ...
    Uint16 pOffTransitoryNoStopKp;          // F9-71  瞬停不停增益
    Uint16 pOffTransitoryNoStopKiCoff;      // F9-72  瞬停不停积分系数
    Uint16 pOffTransitoryNoStopDecTime;     // F9-73  瞬停不停动作减速时间
    Uint16 uVWPGErrorEnable;                // F9-74 UVW编码器故障(Err20)使能
    Uint16 initPosErrorEnable;              // F9-75 初始位置故障使能
    Uint16 encoderBreakEnable;              // F9-76 编码器断线检测使能
    Uint16 overLoderCurve;                  // F9-77 过载曲线
    Uint16 rsvdF9[3];
  ...
  // FC 多端速、PLC
      ...
      Uint16 commOverTime;            // FD-04  串口通讯超时时间
      Uint16 commProtocol;            // FD-05  通讯数据传送格式选择
      Uint16 commReadCurrentPoint;    // FD-06  通讯读取电流分辨率
      Uint16 commMaster;              // FD-07  通讯主从方式
      Uint16 commExtendOverTime;      // FD-08  扩展卡通讯中断检
      Uint16 commSetSpeedType;        // FD-09  通信卡设定速度类型 0 频率 1 转速
...
//==================================================================
  ...
  // A5 控制优化参数
      ...
      Uint16 overModulationFactor;    // A5-05    过调制系数  电流检测延时补偿更改为过调制系数
  ...
  // A8 保留
      Uint16 MSCEnable;               // A8-00 主从控制有效   0:无效   1:有效
      Uint16 slaveEnable;             // A8-01 主从选择       0:主机   1:从机
      Uint16 salveCmdFollow;          // A8-02 从机跟随主机命令
      Uint16 slaveRevDataSel;         // A8-03 从机接收数据   0:转矩给定  1:频率给定
      Uint16 slaveRevOffset;          // A8-04 接受数据零偏
      Uint16 slaveRevGain;            // A8-05 接收数据增益
      Uint16 MSCOverTime;             // A8-06 主从通讯心跳时间(仅主机有效)
      Uint16 MasterSendPeriod;        // A8-07 主从控制通讯发送周期(仅主机有效)
      Uint16 slaveRevFrqOffset;       // A8-08 频率接收数据零偏
      Uint16 slaveRevFrqGain;         // A8-09 频率接收数据增益

      Uint16 slaveTCFrqPosiDev;       // A8-10 从机频率正向最大偏差
      Uint16 slaveWacthWindowFrq;     // A8-11 (保留)从机频率反向最大偏差
  ...
  // AC AIAO校正
      ...
      struct ANALOG_CALIBRATE_CURVE ai2CurrentCalibrateCurve;  // AC-20    ----AC-23, AI2电流校正曲线
      struct ANALOG_CALIBRATE_CURVE ao1CurrentCalibrateCurve;  // AC-24    ----AC-27, AO2电流校正曲线
  ...
  // AE AIAO出厂校正值
      ...
      struct ANALOG_CALIBRATE_CURVE ai2FactoryCurrentCalibrateCurve;   // AE-20    ----AC-23, AI2电流校正曲线
      struct ANALOG_CALIBRATE_CURVE ao1FactoryCurrentCalibrateCurve;   // AE-24    ----AC-27, AO2电流校正曲线
     ...
     // eepromCheckWord
         ...
         Uint16 eepromCheckWord1;        // epromCheckWord1
     ...
     // REMEMBER 掉电保存，共48个
         ...
         Uint16 curABZPulseNumH;
         Uint16 curABZPulseNumL;             // FR-18~FR-19
...
#define FUNCCODE_tuneCmd_ACI_STATIC_WHOLE 3 // 异步机静止完整调谐
...
// AI2模式选择
#define FUNCCODE_ai2ModeSelect_VOL      0   // AI2输入为电压
#define FUNCCODE_ai2ModeSelect_CUR      1   // AI2输入为电流
...
// 设定运行时间到达动作选择
#define FUNCCODE_runTimeOverAct_RUN     0   // 继
...
#define FUNCCODE_pidParaChgCondition_FRQ        3   // 跟据频率渐变
...
// STOP/RES键功能
#define FUNCCODE_stopKeyFunc_KEYBOARD   0   // 停机功能仅在键盘控制
...
#define TIME_UNIT_canLinkOverTime       10      
#define TIME_UNIT_ERR_AUTO_RST_DELAY    100     // 故障自动复位间隔时间，ms
#define TIME_UNIT_ERR_AUTO_RST_CLR      (10UL*TIME_UNIT_MS_PER_SEC*TIME_UNIT_SEC_PER_MIN) // 故障自动复位次数清除时间, 10分钟
...
#define PARA_INIT_MODE_MAX  1002       // 参跏蓟舷拗?  stamp_PI500E
...
#define FUNCCODE_FP_START_INDEX          (GetCodeIndex(funcCode.group.fp[0]))
#define FUNCCODE_FP_END_INDEX            (GetCodeIndex(funcCode.group.fp[FPNUM - 1]))
...
#define CF_00_INDEX   (GetCodeIndex(funcCode.code.cf[0]))

#define TORQUE_BOOST_MOTOR2_INDEX      (GetCodeIndex(funcCode.code.motorFcM2.torqueBoost))    // D0-52   第2电机转矩提升

#define QUICK_CURRENT_LIMIT_INDEX    (GetCodeIndex(funcCode.code.cbcEnable)) // A5-04 逐波限流使能
...
#define SPEED_FILTER_COEFF       (GetCodeIndex(funcCode.code.cf[8]))               // 速度环滤波系数
...
#define AI_AO_CALIB_STOP        (GetCodeIndex(funcCode.code.ao1FactoryCurrentCalibrateCurve.after2))   // aiao厂家校正结束
...
#define AO1_CALB_STOP           (GetCodeIndex(funcCode.code.ao1FactoryCurrentCalibrateCurve.after2)) 
#define TORQUE_UINT    1  // 1 0.0%  10 0.00%
...
//--------------------------------
#if DEBUG_MD500_SEARIS
#define BRAKE_START_VOLTAGE    (GetCodeIndex(funcCode.code.brakeStartVoltage)) 
#elif DEBUG_MD290_SEARIS
#define BRAKE_START_VOLTAGE    (GetCodeIndex(funcCode.code.brakeStartVoltage)) 
#else
#define BRAKE_START_VOLTAGE    (GetCodeIndex(funcCode.code.brakeStartVoltage)) 
//extern const Uint16 uvVoltageInitValue[];
#endif
