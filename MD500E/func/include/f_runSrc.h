...
{                               // bits  description
    ...
    Uint16 onlineTuneSave:1;    // 13    1-在线调谐参数保存
...
extern int32 sCurveA,sCurveFreq;
...
{                               // bits  description
    ...
    Uint16 shortGnd_beforeRun:1;// 14    运行前对地短路检测标志 0:禁止(默认)；1:允许
};
...
{                                       // bits  description
    ...
    Uint16 outPhaseLossProtect_beforeRun:1;   // 2,运行前输出缺相保护
#if DEBUG_MD500_SEARIS
    Uint16 inPhaseLossProtect:1;        // 2:3,    输入缺相保护
#elif DEBUG_MD290_SEARIS
    Uint16 inPhaseLossProtect:2;        // 2:3,    输入缺相保护
#else
    ...
    Uint16 svcSlaveFlag :1;             // 10   主从从机有效标志
};
...
{                                   // bits  description
    ...
    Uint16 overLoadStatus:2;        // 9/10     过载状态   0-正常  1-限转矩
    Uint16 onlineTuneStatus:1;      // 11    参数辨识状态   0-参数辨识没有完成 1-参数辨识完成
};
...
extern int16 brakeUpperTorque;          // 速度控制时为制动转矩上限
extern int16 brakeTorqueAim;            // 速度控制时制动转矩目标设定
