...
#define CAN_OPEN                      2
#define CANLINK                       3
#define EXTEND_COM_CAR                1         // 通信扩展卡

#define RTU_READ_DATA_NUM_MAX     12    // 最多读取数据个数
#define RTU_WRITE_DATA_NUM_MAX     12    // 最多写数据个数
...
struct DP_STATUS_BITS
{
    ...
    Uint16 error:1;            // bit2-变频器是否故障
    Uint16 frqArrive:1;        // bit3-频率到达
    Uint16 dpCommandStatus:1;  // bit4-DP通讯正常
    Uint16 commCtrl:1;         // bit6-变频器控制量为通讯控制
    Uint16 commRun:1;          // bit9-变频器控制命令为通讯控制
    Uint16 speedOrTorque:1;    // bit7-速度控制/转矩控制
    Uint16 errCode:8;          // bit8~bit15 故障代码
#if 0
    Uint16 stopType:1;         // bit8-自由停车/减速停车
    Uint16 isVF:1;             // bit9-VF运行
    Uint16 isSVC:1;            // bit10-SVC运行
    Uint16 isFVC:1;            // bit11-FVC运行
    Uint16 rsvd1:11;           // bit5-bit15
#endif
};

struct DP_STATUS_BITS1
{
    Uint16 run:1;    // bit0-运行
    Uint16 fwd:1;    // bit1-正转
    Uint16 rev:1;    // bit2-反转
    Uint16 err:1;    // bit3-故障
    
    Uint16 noFreeStop:1;    // bit4-非自由停车
    Uint16 DPCommStatus:1;  // bit5-DP通讯正常
    Uint16 Master:1;        // bit6-主机
    Uint16 frqArrive:1;     // bit7-频率到达

    Uint16 fvcFlag:1;  // bit8-是否为闭环矢量
    Uint16 p2pFlag:1;  // bit9-是否处于负荷分配网络
    Uint16 speed:1;    // bit10-速度控制有效
    Uint16 torque:1;   // bit11-转矩控制有效

    Uint16 commRun:1;  // bit12-通讯命令有效
    Uint16 pgError:1;  // bit13-是否为PG故障
    Uint16 rsvd4:1;
    Uint16 rsvd5:1;
};
...
Uint16 CommRwFuncodeAtrrib(Uint16 addr, Uint16 data, Uint16 rwMode);
