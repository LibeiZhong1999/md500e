...
extern Uint16 toDpSpeed; // 传送DP卡的速度
...
void UpdateU0Data(void)
{
    ...
    funcCode.group.u0[5]  = outPowerDisp;                //  5; 输出功率
	...
	funcCode.group.u0[29] = 0;                           // 29; 实际反馈速度
#else
    ...
    funcCode.group.u0[35] = torqueAim/TORQUE_UINT;                   // 35; 目标转矩
    ...
    funcCode.group.u0[46] = dspStatus.all;                 // 46; 主从控制主机发送数据1
    funcCode.group.u0[47] = rsvdData;                      // 47; 主从控制主机发送数据2
    funcCode.group.u0[48] = rsvdData;                      // 48; 主从控制主机发送数据3
    funcCode.group.u0[49] = rsvdData;                      // 49; 主从控制主机发送数据4
    ...
    funcCode.group.u0[58] = motorRs;//ZCount;              // 58; Z信号计数器
    ...
    funcCode.group.u0[62] = errorCode;                     // 62; 当前故障
#if DEBUG_F_MSC_CTRL
    funcCode.group.u0[63] = (int16)itDispP2pSlave;         // 63;
    funcCode.group.u0[64] = MasterSlaveCtrl.slaveNodeNums; // 64; 主从控制主机下属从机个数(主机显示)
#endif
    funcCode.group.u0[65] = upperTorque/TORQUE_UINT;       // 65; 转矩上限
    // 以下扩展卡使用
    funcCode.group.u0[66] = funcCode.code.u3[24];          // 66; 通信控展卡型号
    funcCode.group.u0[67] = funcCode.code.u3[25];          // 67; 通信控展版本号

    funcCode.group.u0[68] = dpStatus.all;                  // 68; DP卡变频器状态
    funcCode.group.u0[69] = toDpSpeed;                     // 69; 传送DP卡的速度 0.01hz
                                                           // 70; 转速 1RMP
    ...
    funcCode.group.u0[72] = funcCode.code.u3[22];           // 72 通讯卡出错状态
    funcCode.code.u0[73]  = motorSn;                        //    电机序号
    funcCode.group.u0[74] = onlineCoeff;                    // 74 在线辨识的反电动势
    funcCode.group.u0[75] = dspStatus.bit.overLoadStatus;   // 75 限转矩模式查看
    funcCode.group.u0[76] = overLoaderPercent;							// 76 过载累计量
