...
// S曲线
//
// Time-stamp: <2008-02-28 20:46:14  Shisheng.Zhi, 0354>
//
//======================================================================
...
LOCALF int32 SCurveEOridinaryCalc(int32 frq0, int32 accTime,int32 decTime);
...
void AccDecFrqCalc(int32 accTime, int32 decTime, Uint16 mode)
{
    ...
    else
    {
        ...
        else if(2 == mode) // alphfa 6000
        {
            frq0 = SCurveEOridinaryCalc(frq0,accTime,decTime);
        }
...
// 动态S曲线
// 2ms调用
// sCurveA/sCurveFreq需要停机清零。
// frq0 单位0.01hz 
//-----------------------------------------------------------------------//
int32 sCurveA;      // 加速度
int32 sCurveFreq;   // s曲线计算输出频率
LOCALF int32 SCurveEOridinaryCalc(int32 frq0, int32 accTime,int32 decTime)
{		
	static int32  Amax = 0;// 最大加加速度
	static int32  aa = 0;  // 加加速度
	static Uint32 freqS = 0;
	static int64 temp1 = 0;
	static int64 temp2 = 0;
    static int32 sFreq = 0;
    static int32 sFreqSet1 = 0;
    static int32 sFreqSetBak = 0;
    static Uint16 bitFlag = 0;
    static Uint16 sAccdectime = 0;
    static Uint16 startTime = 0; // 起始段时间
    static Uint16 endTime   = 0; // 结束段时间
    static int16 signedFlag = 0;
    static int16 signedFlagBk = 0;
#define Q    16  
    sFreq = frq0; // 
    ...
    signedFlagBk = signedFlag;
	if((sFreq != sFreqSet1) || (sCurveA != 0))
	{
		if(sFreqSetBak == sFreqSet1)
		{
			//bitFlag.1 =0  当前频率设定没变 
			bitFlag&=~0x02;
		}
		else
		{
			//bitFlag.1 =1  当前频率设定改变
			bitFlag|=0x02;
			bitFlag&=~0x04; //频率设定改变后没经历过加速阶段
		}
		//备份设定频率
		sFreqSetBak = sFreqSet1;
		freqS = ABS_INT32((sFreqSet1<<Q)-sCurveFreq);
		
		if(sCurveA == 0)
		{
			if(ABS_INT32(sFreqSet1) > ABS_INT32(sFreq))
			{
				//bitFlag.0 =0  频率上升标志 
				bitFlag&=~0x01;
			}
        ...
        else
        {
            //减速
            sAccdectime = decTime;
            startTime   = funcCode.code.sCurveEndPhaseTime;
            endTime     = funcCode.code.sCurveStartPhaseTime;
        }
        // 限幅
        if(!startTime) startTime = 1;
        if(!endTime) endTime = 1;
        
		Amax = (((int64)funcCode.code.maxFrq<<Q))/((Uint32)sAccdectime*(timeBench / RUN_CTRL_PERIOD));//0.01hz/2ms  Q16
		aa = ((int64)Amax*1000)/((Uint32)sAccdectime*endTime*(timeBench / RUN_CTRL_PERIOD));
		temp1 = (int64)sCurveA*sCurveA;
		temp2 = (int64)freqS*2*aa;
		if( (temp1 >= temp2) 
          || (((bitFlag&0x01) == 0)&&(ABS_INT32(sFreqSet1) <= ABS_INT32(sFreq))) 
          ||(((bitFlag&0x01) == 1)&&(ABS_INT32(sFreqSet1) >= ABS_INT32(sFreq))))
		{
			if(sCurveA > aa)
			{
                if((bitFlag&0x01) == 0)
                {
					sCurveA -= aa;
					sCurveFreq += sCurveA*signedFlag;
                }
                else
                {
                    sCurveA -= aa;
                    if(ABS_INT32(sCurveFreq) <= (sCurveA))
                    {
                        sCurveFreq = 0;
                    }
                    else
		...
		else
		{
			//经历加速阶段
			bitFlag|=0x04;
			aa = ((int64)Amax*1000)/((Uint32)startTime*sAccdectime*(timeBench / RUN_CTRL_PERIOD));
            if((bitFlag&0x01) == 0)
            {
				if(sCurveA >= Amax)
				{
		...
		if(((((bitFlag&0x01) == 0)&&(ABS_INT32(sFreq) >= ABS_INT32(sFreqSet1)))||(((bitFlag&0x01) == 1)&&(ABS_INT32(sFreq) <= ABS_INT32(sFreqSet1)))) 
           && ((bitFlag&0x04) == 0x04))
		{
			sFreq = sFreqSet1;
			sCurveFreq = sFreqSet1<<16;
			sCurveA = 0;
		}			
