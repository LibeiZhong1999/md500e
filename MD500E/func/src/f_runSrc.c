...
// 运行命令，frq
//
// Time-stamp: <2008-06-14 16:04:40  Shisheng.Zhi, 0354>
//
//======================================================================
...
extern Uint16 vfTransitoryNoStopFlag;
...
void UpdateRunCmd(void)
{
    ...
    Uint16 diControlMode;
	Uint16 k = 0;
    static Uint16 tuneCmdOldAvlue = 0;
    static Uint16 tuneCmdChFlag = 0;
    jogCmdLevel = 0;
    protectCmd = 0;
...
// 调谐选择有效，面板启动调谐始终有效(不分命令源)
    if(tuneCmd)
    {
        if((KEY_STOP == keyFunc) 
            || (diFunc.f2.bit.stopPanel))
        {
            runCmd.bit.common0 = 0;             // 停机
            runFlag.bit.panelTune = 0;        // 标志清零
        }
        else if (KEY_RUN == keyFunc)
        {
            runCmd.bit.common0 = 1;             // 运行
            runFlag.bit.panelTune = 1;        // 是面板启动调谐
        }
    }
    else
    {
    ...
    switch (runSrc)                     // 运行命令通道
    {
        ...
        case FUNCCODE_runSrc_DI:                        // 运行命令给定方式: 端子
			...
			if((!runCmd.bit.common0)&&(!runFlag.bit.run)) // 停车后不记录端子方向
			{
				runCmd.bit.dir = FORWARD_DIR;
			} 
            jogCmdPulse = 0;

// 面板启动调谐后，运行指令为端子的Case则不再往下执行而直接Break;
// 如果没有此处理，继续往下进入三线\两线式控制处理，runCmd.bit.common0总是为1
            if(runFlag.bit.panelTune) // 是面板启动调谐
            {
                break;
            }
            
            diControlMode = funcCode.code.diControlMode;

            // 三线式\两线式切换
            if (diFunc.f2.bit.twolineOrthreeline)
            {
                if (diControlMode > FUNCCODE_diControlMode_2LINE2)
                {
                    diControlMode -= FUNCCODE_diControlMode_3LINE1;
                }
                else
                {
                    diControlMode += FUNCCODE_diControlMode_3LINE1;
            ...
            switch (diControlMode)        // 端子命令方式

            {
                case FUNCCODE_diControlMode_2LINE1:     // 两线式1
                case FUNCCODE_diControlMode_2LINE2:     // 两线式2
                    if (FUNCCODE_diControlMode_2LINE1 == diControlMode)   // 两线式1
                    {
                        if (fwd && (!rev))              // fwdRun
                        {
                            runCmd.bit.dir = FORWARD_DIR;
                ...
                case FUNCCODE_diControlMode_3LINE2:         // 三线式2
                    if (fwdOld != fwd)  // 脉冲(上升沿)有效
                    {
                        if (fwd)        // fwd
                        {
                            ...
                            if (FUNCCODE_diControlMode_3LINE1 == diControlMode)
                            {
                                runCmd.bit.dir = FORWARD_DIR;
                            }
                        }
                    ...
                    if (FUNCCODE_diControlMode_3LINE1 == diControlMode) // 三线式1
                    {
                        if (revOld != rev)
                        {
                            if (rev)
			...
			// 端子调谐完成之后，如果端子运行信号一直存在，停车。
			// add by 1513 2013-01-21
            if((tuneCmdOldAvlue!=tuneCmd)&&(!tuneCmd)&&(runCmd.bit.common0)) // 调谐信号改变并且调谐无效
            {
				tuneCmdChFlag = 1;										     // 调谐运行-->结束
            }
			else if(tuneCmdChFlag)											 //  
			{
				otherStopSrc++;												 // 保护运行，调谐完停车，
				if(runCmd.bit.common0==0)									 // 端子需再次触发才能运行
				tuneCmdChFlag = 0;
			}
            tuneCmdOldAvlue = tuneCmd;
            break;
#endif
        ...
        case FUNCCODE_runSrc_COMM:                 // 运行命令给定方式: 串口通讯
            ...
            {
                // 主从控制时,仅响应上位机的停机、故障复位指令
                // 当上位机发送停机⒐收细次恢噶钍?从机立即响应
                // 当上位机务指令或运行指令时，从机不响应且从机跟随主机状态运行
                if (commRunCmd < SCI_RUN_CMD_FREE_STOP)
                {

                    // 跟随主机状态运行/停机
                    if (MasterSlaveCtrl.SlaveRcvStatus)
                    {
                        commRunCmd = SCI_RUN_CMD_FWD_RUN;     // 从机运行
                    }
                    else
                    {
                        commRunCmd = SCI_RUN_CMD_FREE_STOP;   // 从机自由停机
                    }
                }
            }
#endif

#if 1       //通讯模式下只需写入调谐参数就自动调谐运行  by modfied yz1990 2014-08-25
			k++;
			if(((tuneCmd == 11)||(tuneCmd == 12))&&(k==1))
			{
			     // funcCode.code.tuneCmd = 1;
			     // commRunCmd = funcCode.code.tuneCmd;
				 commRunCmd = 1;
...
// 通讯复位总是生效(不分命令源)
    if(commRunCmd == SCI_RUN_CMD_RESET_ERROR)
    {
        runCmd.bit.errorReset = 1;
    }
...
// common run
        ...
        )
    {
		...
		else if (runFlag.bit.jog && (!runCmd.bit.jog))
		{
			common = 1;
		}
...
//        || ((RUN_MODE_TORQUE_CTRL == runMode) && (!runCmd.bit.common0)) // 转矩控制模式下，停机命令
        || (funcCode.code.checkDigitalTube)  //检测数码管显示状态下不可运行
        )
    {
    ...
    if ((runFlag.bit.common) 
        ...
        && (!diFunc.f2.bit.brake)                                 //  不为端子立即制动状态
        && (!diFunc.f2.bit.decBrake)                              //  不为端子减速停车制动 
        )
    {
        runCmd.bit.freeStop = 1;
    }
...
// jog when run, 判断是否运行中点动
	...
	{
        runFlag.bit.jogWhenRun = 0;// 同时运行和点动都有效&运行点动优先(运行时，运行和点动同时无效，清运行中点动命令)//yz1990 2015-06-05
	}
...
void RunSrcUpdateFrq(void)
{
    ...
    else if (diFunc.f2.bit.decBrake)
    {
        ...
        if ((runFlag.bit.run)&&(runStatus != RUN_STATUS_DI_BRAKE)) // 运行 且还没制动
        {
    ...
    else if (runCmd.bit.common)
    {
		...
		else if ((runStatus == RUN_STATUS_JOG) && (!runCmd.bit.jog))
		{
...
// runStatus处理
    if (!shuntFlag)      // 没有进入瞬停不停，才处理runStatus
    {
        switch (runStatus)
        {
            ...
            case RUN_STATUS_DI_BRAKE:  // 制动
				...
				runFlag.bit.jog= 0;        //当点动过程没有结束进入直流制动时点动运行标志要清零，
                                            //否则频率源为PID时，直流制动结束后不能启动
    ...
    if((funcCode.code.vcparaM1.onlineTuneEnable != 0)
        &&(motorCtrlTuneStatus == TUNE_STEP_END)
        &&(runCmd.bit.onlineTuneSave == 0))
    {
        runCmd.bit.onlineTuneSave = 1;
        SaveTuneData();
    }
...
// 给真正的frq赋值
    if (RUN_STATUS_POS_CTRL == runStatus)
    {
        ...
        if ((frq > (int32)upperFrq + 500) // 限幅，防止错�
...
LOCALF void StartRunCtrl(void)
{
    ...
    // 增加在线参数辨识完成后再给定频率功能 modify by libo-2103 2014-12-31
    if((dspStatus.bit.onlineTuneStatus == 0)&&(funcCode.code.motorCtrlMode == FUNCCODE_motorCtrlMode_SVC)
    &&(FUNCCODE_startMode_SPEED_TRACK != funcCode.code.startMode))
    {
        return;
    }
        ...
        case START_RUN_STATUS_HOLD_START_FRQ:
                 ...
                 * (Uint16)(TIME_UNIT_START_FRQ_WAIT / RUN_CTRL_PERIOD))
            {
                ...
                if ((FUNCCODE_motorCtrlMode_FVC == motorFc.motorCtrlMode)
                    ||((FUNCCODE_motorCtrlMode_SVC == motorFc.motorCtrlMode)&&(funcCode.code.startMode == 1)))
                {
                    // 从当前电机反馈速度开始启动
...
//    static int16 upperTorqueBak;
...
#if 0
    if ((CanRxTxCon == P2P_COMM_SLAVE)   // 主从有效并且点对点通讯从机有效
        && (funcCode.code.p2pEnable == 1)
        )
    { 
       		// 不处理0转矩封锁PWM，因赝槐淦捣惫�0时，就出现误停车 
			// SVC 转矩低于10%无法启动
    ...
    {    
	    // 当前上限转矩为0且前一拍不为0
	    if ((!upperTorque) && upperTorqueBak) 
	    {
	        dspMainCmd.bit.run = 0;
	        frqTmp = 0;
	    }
	    // 前一拍上限转矩为0且当前不为0
	    else if ((!upperTorqueBak) && upperTorque)
	    {
	        dspMainCmd.bit.run = 1;
...
LOCALF void StopRunCtrl(void)
{
    ...
    if (vfSeparateFlag && funcCode.code.vfSeparateStopMode)  // VF分离有效且停机方式为先停电压
    {
        if (vfSeparateVol > 0)                               // 电压不为0时先不减电压
            return;
    }
...
//                && funcCode.code.stopBrakeTime                       // 且停机直流制动时间不为0
                && enableStopBrake)                    // DI端子的停机直流制动使能端子有效(默认有效)
            {
				...
				if(funcCode.code.accDecSpdCurve == 2)//动态S曲线 //modfied by yz1990 2014-06-04
           	    {
			        sCurveA = 0;      // 加速度    
                    sCurveFreq = 0;   //直流制动结束启动时要把动态S曲线这两值清零，否则在正反转切换中间有直流制动时启动这两值很大，频率失控
			    }
            }
            else
            {
...
LOCALF void TuneRunCtrl(void)
{
    ...
    Uint16 TuneRunTime;
	#define TUNE_RUNTIME_MROE    90  // 90S余值 
#if 0
    ...
    dspMainCmd.bit.run = 1;
#if 1
	TuneRunTime = (accFrqTime + decFrqTime)/(timeBench/10); // 获取加减速总时间
    TuneRunTime = (Uint32)TuneRunTime * motorFc.motorPara.elem.ratingFrq/maxFrq * 4/5; //实际运行时间

    if (++runTimeTicker >= (Uint32)(TuneRunTime + TUNE_RUNTIME_MROE) * (Uint16)(TIME_UNIT_MS_PER_SEC / RUN_CTRL_PERIOD)) // 限时
    {
...
LOCALF void LoseLoadRunCtrl(void)
{
    ...
    else
    {
        ...
        //先计算目标频率frqCurAim，经过加减速时间才得到瞬时频率frqTmp
            if (ABS_INT32(frqAim) > tmp)    // 设定频率大于阈值
            {
                frqCurAim = (FORWARD_DIR == runFlag.bit.dirFinal)? (tmp) : (- tmp);
            }
            else  //否则，为设定频率
            {
                frqCurAim = frqAim;
...
//            && (!runFlag.bit.jog) 非点动
            )
        {
            ...
            {
                plcStepRemOld = plcStep;    // 运行暂停，保鎝lcStep和plcTime
...
//    Uint16 ratingUdc;           // 220V/380V时的标准(额定)母线电压
    ...
    if((funcCode.code.pOffTransitoryNoStop == 2) //瞬停不停减速停机
             && (vfTransitoryNoStopFlag == 2)) //性能传递过来的瞬停不停减速停机标志
	       
	{
		shuntFlag = 1;
		frqCurAim = 0;      // 减速的目标频率为0
		//decTime = funcCode.code.decTime4;
        AccDecFrqCalc(accFrqTime, funcCode.code.pOffTransitoryNoStopDecTime, ACC_DEC_LINE);
        if (0 == frqRun)        // 停机
        {
            runCmd.bit.common = 0;      // 速度为零停机
	//		runCmd.bit.common0 = 0;      // 速度为零停机
	//		runFlag.bit.run = 0;
			shuntFlag = 0;
			otherStopSrc++;
        }
	}
	else     //funcCode.code.TransitoryNoStopEnable为0，1时不用管，直接性能控制
	{
	    shuntFlag = 0;
	}
...
void AccDecTimeCalc(void)
{
    ...
    {
        accFrqTime = funcCode.code.accTime4;  // 转矩控制加速时间为0
        decFrqTime = funcCode.code.decTime4;  // 转矩控制减速时间为0
    }
    else
    {
