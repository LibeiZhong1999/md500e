...
Uint16 ShieldMenu3Number(Uint16 index, Uint16 *data, int16 Delta);
//Uint16 ShieldMenu2Number(Uint16 index, Uint16 *data, int16 Delta);
Uint16 ShieldMenu2Number(int16 Delta);
...
// 须放在 功率改变处理 和 机型改变处理 后面。
    ...
    //MD290进入3及菜单按ENTER键后需跳过的功能码
#if DEBUG_MD290_SEARIS
    if(menuLevel == MENU_LEVEL_2)
    {
        ShieldMenu2Number(ON_UP_KEY);
    }
...
Uint16 ValidateTuneCmd(Uint16 value, Uint16 motorsN)
{
...
#endif
    {
        if ((FUNCCODE_tuneCmd_ACI_STATIC != value) &&
            ...
            (FUNCCODE_tuneCmd_ACI_STATIC_WHOLE != value)
            )
        {
            return COMM_ERR_PARA;
        }
...
//                menuAttri[MENU_LEVEL_0].winkFlag = 0;        // 在0级菜ハ掳聪耈P/DOWN时, 之后_时间也不闪烁
        ...
        else
        {
                ...
                )
            {
                attributeMenu0.bit.point = funcCode.code.frqPoint;
#if 0
                if(DISP_FRQ_RUN_FDB == bitDisp)
                {
                    attributeMenu0.bit.point = funcCode.code.speedDispPointPos;
                    if((attributeMenu0.bit.point) // 显示两位小数超过320.00hz时自动切换成1位小数 by modfied yz1990 2014-07-14
                    && ((int16)frqRunDisp < (-3200))
                    &&((funcCode.code.speedDispPointPos/10) == 2)
                    )
                    {
                        attributeMenu0.bit.point--;
                    }
                    else if((attributeMenu0.bit.signal) // 显示两位小数超过320.00hz时自动切换成1位小数 by modfied yz1990 2014-07-14
                    && (frqRunDisp > 3200)
                    && ((funcCode.code.speedDispPointPos/10) == 2)
                    )
                    {
                        if(frqRunDisp >= (Uint32)3200)
                        {
                            attributeMenu0.bit.point--;
                            //dataTmp = data/10;
                        }
            ...
            {
                attributeMenu0.bit.point = funcCode.code.speedDispPointPos%10; // 速度显示小数点位
            }
...
// 380目前，不可通讯调谐
    ...
    {
        attribute.bit.point = funcCode.code.speedDispPointPos%10;
    }  
    else if((GetCodeIndex(funcCode.group.u0[DISP_FRQ_RUN_FDB]) == curFc.index)
            ||(GetCodeIndex(funcCode.group.u0[DISP_FRQ_FDB]) == curFc.index))
    {
        attribute.bit.point = funcCode.code.speedDispPointPos/10;

// 显示两位小数超过320.00hz时自动切换成1位小数 by modfied yz1990 2014-07-14
        if(curFc.grade == 19)
        {
            if((attribute.bit.signal) // 显示两位小数超过99.99hz(和负方向(-99.99)保持对称)时自动切换成1位小数 by modfied yz1990 2014-07-14
               && (ABS_INT32(frqRun) > 9999)
               && ((funcCode.code.speedDispPointPos/10) == 2)
               )
            {
                attribute.bit.point--;
                tmp = funcCode.group.u0[19];
            }

        }
        else if(curFc.grade == 29)
        {
            if((attribute.bit.signal) // 显示两位小数超过320.00hz时自动切换成1位小数 by modfied yz1990 2014-07-14
               && (ABS_INT32(frqFdbTmp) > 9999)
               && ((funcCode.code.speedDispPointPos/10) == 2)
               )
            {
                attribute.bit.point--;
                tmp = funcCode.group.u0[29];
            }
        }
...
// 额定电流，空载电流，故障电流，U0-04显示电流
        ...
        (GetCodeIndex(funcCode.code.errorScene1.elem.errorCurrent) == curFc.index) ||
...
// 用户定制功能码组的显示
    ...
    if (((MENU_LEVEL_0 == menuLevel) // 0级菜单，不显示数字前面的零
        || ((MENU_LEVEL_3 == menuLevel) && (curFc.group >= FUNCCODE_GROUP_U0)) // U组，显示
            ...
            || (GetCodeIndex(funcCode.code.softVersion) == curFc.index)
            || (GetCodeIndex(funcCode.code.tempsoftversion) == curFc.index)
            || (GetCodeIndex(funcCode.code.coreSoftVersion) == curFc.index)
            || (GetCodeIndex(funcCode.code.CoreTempSoftversion) == curFc.index)
            )
    {
        if (mode == HEX)
        {
...
// 软件版本号特殊显示
    if ((curFc.index == GetCodeIndex(funcCode.code.softVersion))
        && (MENU_LEVEL_3 == menuLevel)
        )
    {
        switch (softVersionViewType)
        {
            case VIEW_TYPE_STANDAR:          // 通用软件v开头
                displayBuffer[4-digits] = DISPLAY_CODE[DISPLAY_u];
                break;

            case VIEW_TYPE_TEMP:             // 临时软件F开头
                displayBuffer[4-digits] = DISPLAY_CODE[DISPLAY_L];
                break;
#if 0
            case VIEW_TYPE_NON_STANDAR:      // 非标软件直接显示版本数字
                break;
#endif
            default:
                break;
        ...
        attribute.bit.point = softVersionVierPoint;
            
    }
// 性能软件版本号特殊显示
    else if ((curFc.index == GetCodeIndex(funcCode.code.coreSoftVersion))
        && (MENU_LEVEL_3 == menuLevel)
        )
    {
        switch (coreSoftVersionViewType)
        {
            case VIEW_TYPE_STANDAR:          // 通用软件v开头
                displayBuffer[4-digits] = DISPLAY_CODE[DISPLAY_u];
                break;
        ...
        attribute.bit.point = coreSoftVersionVierPoint;
...
// 2. 显示小数点
    if (attribute.bit.point)
    {
        displayBuffer[(DISPLAY_8LED_NUM - 1) - attribute.bit.point] &= DISPLAY_CODE[DISPLAY_DOT];
            // 版本号功能码显示  最左边小数点点亮 add by zhurenfei 1513
        //if(attribute.bit.point==5)
        //{
        //    displayBuffer[(DISPLAY_8LED_NUM - 1) - 2] &= DISPLAY_CODE[DISPLAY_DOT];
        //    displayBuffer[(DISPLAY_8LED_NUM - 1) - 4] &= DISPLAY_CODE[DISPLAY_DOT];
        //}
    }
...
#if DEBUG_MD290_SEARIS                        //by modfied yz1990 2014-06-24
            Validate290InvType_InvGpType();   //MD290的G/P机型和机型(功率)由F0-00和FF-02共同决定，F0-00优先级比FF-02高
#endif                                        //通过改变G/P机型和机型(功率)来选择过载曲线和机型(功率)

        }
#endif

#if DEBUG_MD290_SEARIS       //by modfied yz1990 2014-06-24 //290的F0-00可选择，只作为改变负载曲线选择
        else if (GetCodeIndex(funcCode.code.inverterGpTypeDisp) == index) // 修改变频器机型 FF-01，立即修改相关功能码
        {
             Validate290InvType_InvGpType();
        }

#endif

//G/P型机型FF-02改变时修改現9-06出厂值  //by modfied yz1990
        else if (GetCodeIndex(funcCode.code.inverterGpType) == index)
        {
             if(funcCode.code.inverterGpType == 1)
             {
                 funcCode.code.ocPoint = 150;
             }
             else
             {
                  funcCode.code.ocPoint = 130;
             }
             SaveOneFuncCode(GetCodeIndex(funcCode.code.ocPoint));
        }

//F9-08是F9-04和F3-22的上下限处理,不放入LimitOtherCodeDeal(index);// 某些功能码是其他功能码上下限的处理

#if 0
#if DEBUG_MD500_SEARIS  //F9-08是F9-04的上限 //by modfied yz1990
        else if(GetCodeIndex(funcCode.code.brakeStartVoltage) == index)
        {
            if(funcCode.code.ovPoint > funcCode.code.brakeStartVoltage)
            {
                funcCode.code.ovPoint = funcCode.code.brakeStartVoltage;
...
// 放在UpdateFrqAim()中。
    ...
    if (runFlag.bit.run)
    {
        loadSpeedDispTmp = (ABS_INT32(frq) * funcCode.code.speedDispCoeff) / 10000;
    }
    else
    {
        loadSpeedDispTmp = (ABS_INT32(frqAim) * funcCode.code.speedDispCoeff) / 10000;
    }
    if(loadSpeedDispTmp >= 65535)               // 防止U0-14显示溢出
    {
        loadSpeedDispTmp = 65535;
    }
    loadSpeedDisp = loadSpeedDispTmp;
    
    pulseInFrqDisp = (pulseInFrq + 5) / 10;

    //torqueCurrentAct = (Uint32)torqueCurrent * currentPu >> 12;
    torqueCurrentAct = ABS_INT32(((int32)(int16)torqueCurrent * currentPu >> 12));
...
#if DEBUG_MD290_SEARIS         // 290的F0-00可修改，改变过载曲线 by modfied yz1990 2014-06-24
...
// F0,…,FE,FF,FP,A0,…,AF,B0,…,BF,C0,…,CF,
    ...
    else if (group <= FUNCCODE_GROUP_CF)    // A0,…,AF,B0,…,BF,C0,…,CF,
    {
        ...
        currentGroupDispF = DISPLAY_A + tmp1;
        currentGroupDisp0 = tmp2;
    }
    else    // U组
    {
        currentGroupDispF = DISPLAY_U;
...
void UpdataFuncCodeGrade(Uint16 funcCodeGrade[])
{
    ...
    {
        funcCodeGrade[FUNCCODE_GROUP_AE] = AENUM - 8;
    }
#endif
...
//====================================================================
// 按位(个位、十位、百位、千位、万位)修改的十进制功能码，转换为二进制
// 每位仅能为0, 1
...
Uint16 FcDigit2Bin(Uint16 value)
{
    ...
    for (i = 5-1; i >= 0; i--)
    {
        if (digit[i])       //if (1 == digit[i])    // 如果该位为1.
        {
            tmp += 1 << i;
        }
    }
...
{
    //对F1-00值为2的屏蔽
    if((index == GetCodeIndex(funcCode.code.motorParaM1.elem.motorType))
       ||(index == 0x0100)||(index == 0xf100)
      )
    {
        if((Delta > 0) && (*data == 2))
        {
    ...
    //对A5-07值为0,1的屏蔽
    if((index == GetCodeIndex(funcCode.code.svcMode))
       ||(index == 0xA507)||(index == 0x4507)
      )
    {  
        if(Delta > 0) 
        {
    ...
    //对F0-01值为1是的屏蔽
    if((index == GetCodeIndex(funcCode.code.motorCtrlMode)) 
        ||(index == 0001)||(index == 0xf001)
      )
    {
        if((Delta > 0) && (*data == 1))
        {
    ...
    //对F1-37值为1，2的屏蔽
    if((index == GetCodeIndex(funcCode.code.tuneCmd))
       ||(index == 0x0125)||(index == 0xf125)
      )
    {
        if((Delta > 0) && (*data == 1))
        {
    ...
    //对F6-07值为1，2的屏蔽
    if((index == GetCodeIndex(funcCode.code.accDecSpdCurve))
       ||(index == 0x0607)||(index == 0xf607)
      )
    {
        if((Delta > 0) && (*data == 1))
        {
...
//Uint16 ShieldMenu2Number(Uint16 curFcGroup,Uint16 curFcGrade)
Uint16 ShieldMenu2Number(int16 Delta)
{
    curFc.index = GetGradeIndex(curFc.group, curFc.grade);
//跳过F9-03~F9-06
    if((curFc.index ==  GetCodeIndex(funcCode.code.ovGain))&&(Delta == ON_UP_KEY))
    {
        if(Delta == ON_UP_KEY)
        {
            curFc.grade +=4;
    ...
    else if((curFc.index ==  GetCodeIndex(funcCode.code.ocPoint))&&(Delta == DEC_SPEED))
    {
         if(Delta == DEC_SPEED)
        {
            curFc.grade -=4;
    ...
    else if((curFc.index >= GetCodeIndex(funcCode.code.ovGain))
        &&(curFc.index <= GetCodeIndex(funcCode.code.ocPoint))
        )
    {
        curFc.grade = 2;
    }
//跳过F1-11~F1-36
    else if((curFc.index ==  GetCodeIndex(funcCode.code.motorParaM1.elem.rsvdF11[0]))&&(Delta == ON_UP_KEY))
    {
        if(Delta == ON_UP_KEY)
        {
            curFc.grade +=26;
    ...
    else if((curFc.index ==  GetCodeIndex(funcCode.code.pgParaM1.elem.fvcPgLoseTime))&&(Delta == DEC_SPEED))
    {
         if(Delta == DEC_SPEED)
        {
            curFc.grade -=26;
    ...
    else if((curFc.index >= GetCodeIndex(funcCode.code.motorParaM1.elem.rsvdF11[0]))
        &&(curFc.index <= GetCodeIndex(funcCode.code.pgParaM1.elem.fvcPgLoseTime))
        )
    {
        //curFc.grade = 26;
        if(Delta == ON_UP_KEY)
        ...
        else if(Delta == DEC_SPEED)
        {
            curFc.grade = 10;
        }
    }
//跳过F9-51~F9-53
    else if((curFc.index ==  GetCodeIndex(funcCode.code.errorAction[4]))&&(Delta == ON_UP_KEY))
    {
        if(Delta == ON_UP_KEY)
        {
            curFc.grade +=3;
    ...
    else if((curFc.index ==  GetCodeIndex(funcCode.code.errorShow[1]))&&(Delta == DEC_SPEED))
    {
         if(Delta == DEC_SPEED)
        {
            curFc.grade -=3;
    ...
    else if((curFc.index >= GetCodeIndex(funcCode.code.errorAction[4]))
        &&(curFc.index <= GetCodeIndex(funcCode.code.errorShow[1]))
        )
    {
        curFc.grade = 50;
    }
...
//跳过F9-67~F9-70
    else if((curFc.index ==  GetCodeIndex(funcCode.code.osChkValue))&&(Delta == ON_UP_KEY))
    {
        if(Delta == ON_UP_KEY)
        {
            curFc.grade +=4;
    ...
    else if((curFc.index ==  GetCodeIndex(funcCode.code.devChkTime))&&(Delta == DEC_SPEED))
    {
         if(Delta == DEC_SPEED)
        {
            curFc.grade -=4;
    ...
    else if((curFc.index >= GetCodeIndex(funcCode.code.osChkValue))
        &&(curFc.index <= GetCodeIndex(funcCode.code.devChkTime))
        )
    {
        curFc.grade = 66;
    }
//跳过A2-11~A2-36
    else if((curFc.index ==  GetCodeIndex(funcCode.code.motorFcM2.motorPara.elem.rsvdF11[0]))
        &&(Delta == ON_UP_KEY))
    {
        if(Delta == ON_UP_KEY)
        {
    ...
    else if((curFc.index ==  GetCodeIndex(funcCode.code.motorFcM2.pgPara.elem.fvcPgLoseTime))
        &&(Delta == DEC_SPEED))
    {
         if(Delta == DEC_SPEED)
        {
        ...
        &&(curFc.index <= GetCodeIndex(funcCode.code.motorFcM2.pgPara.elem.fvcPgLoseTime))
        )
    {
        //curFc.grade = 26;
        if(Delta == ON_UP_KEY)
...
//跳过A2-38~A2-55
    else if((curFc.index ==  GetCodeIndex(funcCode.code.motorFcM2.vcPara.vcSpdLoopKp1))
        &&(Delta == ON_UP_KEY))
    {
        if(Delta == ON_UP_KEY)
        {
    ...
    else if((curFc.index ==  GetCodeIndex(funcCode.code.motorFcM2.vcPara.spdLoopI))
        &&(Delta == DEC_SPEED))
    {
         if(Delta == DEC_SPEED)
        {
        ...
        &&(curFc.index <= GetCodeIndex(funcCode.code.motorFcM2.vcPara.spdLoopI))
        )
    {
...
//跳过A5-07
    else if((curFc.index ==  GetCodeIndex(funcCode.code.svcMode))&&(Delta == ON_UP_KEY))
    {
        if(Delta == ON_UP_KEY)
        {
            curFc.grade +=1;
    ...
    else if((curFc.index ==  GetCodeIndex(funcCode.code.svcMode))&&(Delta == DEC_SPEED))
    {
         if(Delta == DEC_SPEED)
        {
            curFc.grade -=1;
    ...
    /*curFc.index = GetGradeIndex(curFc.group, curFc.grade);
    if((curFc.index ==  GetCodeIndex(funcCode.code.ovGain))
       ||(curFc.index ==  GetCodeIndex(funcCode.code.ovPoint))
       ||(curFc.index ==  GetCodeIndex(funcCode.code.ocGain))
       ||(curFc.index ==  GetCodeIndex(funcCode.code.ocPoint))
    )
    {
        if(Delta == ON_UP_KEY)
    ...
    else if(curFc.index ==  GetCodeIndex(funcCode.code.ocPoint))
    {
         if(Delta == DEC_SPEED)
        {
            curFc.grade -=4;
