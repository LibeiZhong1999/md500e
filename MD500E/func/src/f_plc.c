...
#define COMM_READ_CURRENT_FC GetCodeIndex(funcCode.group.d0[4])              // 通讯读取电流
...
* 说明				: 变频器发送表配置
********************************************************************************/
Uint16 InvTranTabCfg(CANlinkDataBuf *dataPi)
{
	Uint16 err;
	err = FuncCodeRwAtrrib(dataPi->data.dByte.dByte1, COMM_READ_FC);
	if (err)
		return (COMM_ERR_PARA); 							// 配置数据错 无效参数

	err = COMM_ERR_ADDR;		                            // 不成功无效地址
	if (dataPi->data.dByte.dByte2 >= INV_SLOW_TAB_ADDR)	// 配置发送慢表
	{
		if ( (dataPi->data.dByte.dByte2 - INV_SLOW_TAB_ADDR) <= INV_TRAN_SLOW_NUM )
		{
			InvTrSlowTab[dataPi->data.dByte.dByte2 - INV_SLOW_TAB_ADDR] = dataPi->data.dByte.dByte1;
			err = COMM_ERR_NONE;							// 操作成功
		}
	}
	else if (dataPi->data.dByte.dByte2 != 1)				// 发送快表，“1”地址固定不允许写操作
	{
		if (dataPi->data.dByte.dByte2 <= INV_TRAN_FAST_NUM)
		{
			InvTrFastTab[dataPi->data.dByte.dByte2] = dataPi->data.dByte.dByte1;
			err = COMM_ERR_NONE;							// 操作成功
		}
	}
...
* 说明				: 变频器接收表配置
********************************************************************************/
Uint16 InvRecTabCfg(CANlinkDataBuf *dataPi)
{
	Uint16 err;
	err = FuncCodeRwAtrrib(dataPi->data.dByte.dByte1, COMM_WRITE_FC);
	if (err)
		return (err);										// 配置数据错

	err = COMM_ERR_PARA;									// 预置参数出错		
	if (dataPi->data.dByte.dByte2 >= INV_SLOW_TAB_ADDR)  // 配置接收慢表
	{
		if ( (dataPi->data.dByte.dByte2 - INV_SLOW_TAB_ADDR) <= INV_TRAN_SLOW_NUM )
		{
			InvRecSlowTab[dataPi->data.dByte.dByte2 - INV_SLOW_TAB_ADDR] = dataPi->data.dByte.dByte1;
			err = COMM_ERR_NONE;							// 操作成功
		}
	}
	else                                                    // 接收快表
	{
		if (dataPi->data.dByte.dByte2 <= INV_TRAN_FAST_NUM)
		{
			InvRecFastTab[dataPi->data.dByte.dByte2] = dataPi->data.dByte.dByte1;
			err = COMM_ERR_NONE;							// 操作成功
		}
	}
	return (err);
...
* 说明				: 处理PLC卡数据帧
********************************************************************************/
void PlcDataFramDeal(CANlinkDataBuf *dataPi)
{
	Uint16 addr, *InvRecTab, *InvTrTab;					    // 收发表指针
	Uint16 result, invTrIndex, indexReset = 1;              // "1"时复引
	Uint16 len = 0;
	
	addr = dataPi->data.dByte.dByte1;		                // 数据地址索引
...
// 接收表格处理	
	...
	{														// 所有表格操作都是写RAM
		WriteSingleReg(InvRecTab[addr], dataPi->data.dByte.dByte2, 0);
		if (InvRecTab[addr+1])
		{
			WriteSingleReg(InvRecTab[addr+1], dataPi->data.dByte.dByte3, 0);		
			if (InvRecTab[addr+2])	
				WriteSingleReg(InvRecTab[addr+2], dataPi->data.dByte.dByte4, 0);			
		}
	}
...
// 发送表格返回
	if (InvTrTab[invTrIndex])
	{
        if (InvTrTab == InvTrFastTab)
            dataPi->data.dByte.dByte1 = invTrIndex;		// 发送数据表地址索引
        else
            dataPi->data.dByte.dByte1 = invTrIndex + INV_SLOW_TAB_ADDR;
        
        ReadSingleReg(InvTrTab[invTrIndex], &result);
		dataPi->data.dByte.dByte2 = result;
		len += 4;
		if (InvTrTab[invTrIndex+1])
		{
			ReadSingleReg(InvTrTab[invTrIndex+1], &result);
			dataPi->data.dByte.dByte3 = result;
			len += 2;
			if (InvTrTab[invTrIndex+2])
			{
				ReadSingleReg(InvTrTab[invTrIndex+2], &result);
				dataPi->data.dByte.dByte4 = result;
				len += 2;
