...
//
// Time-stamp: <2008-10-22 21:06:51  Shisheng.Zhi, 0354>
//
//======================================================================
...
#define DEBUG_F_AI_NEGTIVE_TEN  0
...
#define AI_MAX_CURRENT_HW       2000     // 硬件的AI最大输入电流,取多少电流对应DSP满量程
#define AI_MAX_CURRENT_IDEA     20000    // 理想的AI最大输入电流,单位0.001ma
...
void Dout28(void)
{
    if ((outCurrentDisp >= (((int32)funcCode.code.currentArriveValue1 - funcCode.code.currentArriveRange1)*motorFc.motorPara.elem.ratingCurrent/1000))
        && (outCurrentDisp <= ((Uint32)(funcCode.code.currentArriveValue1 + funcCode.code.currentArriveRange1)*motorFc.motorPara.elem.ratingCurrent/1000))
        && runFlag.bit.run
        )
    {
...
#if DEBUG_MD380E_SEARIS
void Dout42(void)
{
	if(runFlag.bit.accDecStatus == ACC_SPEED)
	{
		doFunc.f2.bit.addSpeedStatus = 1;
	}
}
void Dout43(void)
{
	if(runFlag.bit.accDecStatus == DEC_SPEED)
	{
		doFunc.f2.bit.decSpeedStatus = 1;
	}	
}
typedef struct
{
...
// DO3(FMR)的控制在FMPDeal()中
    ...
    doHwStatus.bit.fan    = ((dspStatus.bit.fan | funcCode.code.fanControl)&(!bUv));
...
//上面都是对MD380的AI3作为温度传感器的采样和校正处理 //modfied by yz1990 2014-07-25
//MD290和MD500的AI3作为温度传感器和AI3作为模拟量时处理一样，在AI作为模拟量函数
#endif

//MD290、MD380、MD500的AI3作为温度传感器和AI3作为模拟量时处理一样，在AI作为模拟量函数
//#if DEBUG_MD500_SEARIS||DEBUG_MD290_SEARIS
        // 限幅，防止在临界出错
        if(aiDeal[2].voltageOrigin < 0)
        {
            aiDeal[2].voltageOrigin = 0;
        }
        if(aiDeal[2].voltage1 < 0)
        {
            aiDeal[2].voltage1 = 0;
        }
		//获取MD290 面MD500 AI3的采样值和校正值直接查表获取PT100，PT1000(硬件扩展卡还不支持)温度传感器温度 //modfied by yz1990 2014-07-25
        temperatureVoltageOrigin = aiDeal[2].voltageOrigin;
        temperatureVoltage =  aiDeal[2].voltage1;
//#endif
...
#define AI3_RESULT  (AdcRegs.ADCRESULT5)    // AI2对应的ADCRESULT
    ...
    //j = 0;
    {
        Uint16 ai2Flag,ai2CurveGain;

        if ((j == 1)                                                     // AI2
            && (funcCode.code.ai2ModeSel == FUNCCODE_ai2ModeSelect_CUR)  // 输入信号为电流
            )
        {
            ai2Flag = 1;
            ai2CurveGain = 20;
...
#if 0
//注意#ifndef是指假如没有定义，和#ifdef的区别 modfied by yz1990 2014-07-25		
#if DEBUG_MD500_SEARIS||DEBUG_MD290_SEARIS  // 新温度校正删除以下程序 //新温度校正不执行下面的if,继续以AI的处理作为温度转换值 
                                            // MD500，MD290 AI3为温度传感器时继续进行AI3的处理，校正后的值转换为温度值
        
#else //MD380 AI3为温度传感器时不进行后面AI3作为模拟量的处理,cotinue跳出,后面在温度传感器函数进行AI3作为PT100采样，校正处理
        if ((j == AI_NUMBER - 1) 
...
// AI滤波
        ...
        aiLine.x2 = (4095L << 4);
        if (ai2Flag)
            aiLine.y2 = (int32)AI_MAX_CURRENT_HW*10;
        else
...
#undef SIZE_AI_CALIBRATE
        if (ai2Flag)
        {
            p = &funcCode.code.ai2CurrentCalibrateCurve.before1;
        }
        ...
        {
            aiDeal[j].voltage = (aiLine.y + 5) / 10;   // 实际电压
        }
//#if DEBUG_MD500_SEARIS||DEBUG_MD290_SEARIS
        aiDeal[j].voltage1 = aiLine.y;    // 再上面这段程序后增加这一行程序即可，先在aiDeal结构体中增加voltage1
//#endif
// 找AI设定曲线
...
// AI设定
        ...
        //aiLine.mode = 0;        // 限幅
        aiLine.x1 = (int32)(int16)(*(p + 0)) * ai2CurveGain;  // 电压点1
        aiLine.y1 = (int32)(int16)(*(p + 1)) * 10;            // 对应物理量1
        aiLine.x2 = (int32)(int16)(*(p + 2)) * ai2CurveGain;  // 电压点2
        aiLine.y2 = (int32)(int16)(*(p + 3)) * 10;            // 对应物理量2
        aiLine.x = aiLine.y;                                  // 校正后电压值 edit by 0656 2012-12-12
        aiLine.calc(&aiLine);
        // aiLine.y 对应物理量100.00%
...
#define SIZE_AI_JUMP sizeof(struct AI_JUMP)     // AI JUMP的size
        ...
        {
            aiDeal[j].set = point * 0x7FFF / 10000;                   // 限幅
            //aiDeal[j].set = point * 13421 >> 12;  // 0x7FFF/10000  // 限幅

        }
        else
        {
            aiDeal[j].set = (int32)aiLine.y * 0x7FFF / 10000;  // 不限幅
            //aiDeal[j].set = (int32)(int16)aiLine.y * 13421 >> 12;  // 0x7FFF/10000  // 限幅
...
// 功能码的零偏和增益
    outCoeff = ((outCoeff * (int32)(int16)funcCode.code.aoPara[channel-1].gain) * 327 >> 15)
        + ((((int32)(int16)funcCode.code.aoPara[channel-1].offset * 2097) >> 6)*1000/1047); //by modfied yz1990 2014-06-30
           //在偏置每增加10%时,输出有5%的偏差(20hz运行，偏置5%,理论9ma,实际9.05ma;偏置10%,理论10ma,实际10.1ma;偏置20%,理论12ma,实际10.20ma)，需要修正
...
// AO校正曲线
    ...
    if ((channel == AOFMP_AO1)                            // AO1
        && (funcCode.code.ao1ModeSel)                     // 为电流输出
        )
    {
        pAo = &funcCode.code.ao1CurrentCalibrateCurve;  // AO1电流曲线
        baseValue = 13;
    }
    else
    {
        pAo = &funcCode.code.aoCalibrateCurve[channel-1];  // 电压曲线
        baseValue = 12;
    }
    
    aoLine.mode = 1;    // 不限幅
    aoLine.x1 = ((int32)(int16)pAo->after1  * 13421 ) >> baseValue;  // 0x7FFF/10000 万用表测量电压
    aoLine.y1 = ((int32)(int16)pAo->before1 * 13421 ) >> baseValue;  // 0x7FFF/10000 理论输出电压
    aoLine.x2 = ((int32)(int16)pAo->after2  * 13421 ) >> baseValue;  // 0x7FFF/10000 万用表测量电压
    aoLine.y2 = ((int32)(int16)pAo->before2 * 13421 ) >> baseValue;  // 0x7FFF/10000 理论输出电压
    aoLine.x = outCoeff;
    aoLine.calc(&aoLine);
    outCoeff = aoLine.y;
...
LOCALF void UpdateFmpAoValue(Uint16 func, Uint16 aoOrFmp)
{
    ...
    switch (func)
    {
        ...
        case AO_FMP_FUNC_SPEED:         // 电机运行转速
            ...
            {
                aoFmpValue = (int16)frqFdb;   // 闭环矢量时为编码器速度反馈
            }
            else
        #endif
            {
        ...
        case AO_FMP_FUNC_OUT_TORQUE_DIR:
            ...
            break;
#if 0
		 // 电机实际输出转矩(带方向) -100.0%~100.0%对应0~10V
        case AO_FMP_FUNC_OUT_FACT_TORQUE_DIR:
			aoFmpValue = (int16)(torqueFactOut)+1000;
            aoFmpMax = 2000;                    // 电机额定转矩
			break;			
 #endif           
        default:
            break;
...
/*    tmp = ((((int32)(p->x - p->x1) << 15) / (p->x2 - p->x1)) * (p->y2 - p->y1)
                 + ((int32)p->y1 << 15) + (1L << 14)) >> 15;
*/
 tmp = ((((int64)(int32)(p->x - p->x1) << 15) / (p->x2 - p->x1)) * (p->y2 - p->y1)
                 + ((int64)(int32)p->y1 << 15) + (1L << 14)) >> 15;
...
EALLOW;
	...
	doHwStatus.bit.fan ? (GpioDataRegs.GPBCLEAR.bit.GPIO37 = 1): (GpioDataRegs.GPBSET.bit.GPIO37 = 1);
