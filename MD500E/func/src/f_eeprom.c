...
//
// 
// Time-stamp: <2008-01-03 14:21:33  Shisheng.Zhi, 0354>
//
//======================================================================
...
//extern const Uint16 eeprom2Fc[994];
#endif
//--------------------------------------
...
#define FUNCCODE_READ_VERIFY_WRONG_TIME_MAX    4        // 4
#define FUNCCODE_READ_AFTER_WRITE_VERIFY_RIGHT_TIME_MAX   2   // 2次
#define FUNCCODE_READ_AFTER_WRITE_VERIFY_WRONG_TIME_MAX   3   // 3
#define FUNCCODE_READ_TIME_MAX                 5 //5
...
{
    funcCodeOneWriteIndex[funcCodeOneWriteNum++] = (index);
}
...
#define EEPROM_RW_FUNCCODE_MAX      8000    // _*2ms，连续读写功能码的超时时间
...
{
    int16 i;
    static Uint16 fcIndex = 0;
    static Uint16 iResty = 0;
...
// 准备要写入的数据
    if (FUNCCODE_RW_STATUS_PREPARE_DATA_FOR_WRITE == funcCodeRwStatus)
    {
        ...
        for (iResty = 0; iResty < number4EepromDeal; iResty++)
        {
            fcIndex = GetFcIndexFromEepromIndex(index4EepromDeal + iResty);  // 获取功能码index
            ...
            {
                funcCode.all[fcIndex] = funcCodeRead.data[iResty];
            }
            ...
            else if (FUNCCODE_paraInitMode_RESTORE_USER_PARA == funcCodeRwMode) // 恢复
            {
                ...
                if((fcIndex <= RUNCCODE_USER_PARA_END_INDEX)
                && !((fcIndex >= FUNCCODE_FACTORY_START_INDEX) && (fcIndex <= FUNCCODE_FACTORY_END_INDEX))
                && !((fcIndex >= FUNCCODE_FP_START_INDEX) && (fcIndex <= FUNCCODE_FP_END_INDEX))
                )
                {
                    funcCode.all[fcIndex] = funcCodeWrite.data[iResty];

                }
            }
#endif

            funcCodeWrite.data[iResty] = funcCode.all[fcIndex];  // 准备要写入的数据

            // 确定是否数据需要写入：全部与当前EEPROM数据相等，则跳过；否则，需要写入。
            // 读取全部功能码，不会有写操作
            if (funcCodeWrite.data[iResty] != funcCodeRead.data[iResty])
            {
                funcCodeRwStatus = FUNCCODE_RW_STATUS_WRITE_DATA;
            }
        }
