...
const Uint16 errorCodeFromMotorList[] =
{
    ...
    ERROR_BRAEK_IGBT_OVERLOAD,// 61制动管过载
    ERROR_BRAEK_IGBT_SHORT,   // 62制动管直通
#else 
    ERROR_RSVD,               // 52 -- 保留
    ...
    ERROR_BRAEK_SHORT,        // 60 -- 制动电阻短路
#if DEBUG_MD380E_SEARIS
    ERROR_BRAKE_R_DETECT,     // 61 -- 制动电阻连续开通超时
#else
    ERROR_RSVD,               // 63 -- 预留
#endif
	ERROR_COEFF               // 64 -- 反电动势异常故障
...
void getMotorError(void)
{
#if DEBUG_F_ERROR_FROM_MOTOR
    Uint16 errorStatus;
    Uint16 motorErrorCodeTmp, motorErrorCode;
    Uint16 errorAttributeTmp, errorAttribute; 
    Uint16 motorErrorInfo;
    Uint16 i;
    
    // 初始为无故障
    motorErrorCode = ERROR_NONE;
    errorAttribute = GetErrorAttribute(motorErrorCode);
    ...
    if (errorsCodeFromMotor[0] || errorsCodeFromMotor[1])
    {
        for (i = 0; i < sizeof(errorCodeFromMotorList); i++)
        {
            ...
            {
                // 获得有效故障的故障编码
                motorErrorCodeTmp = errorCodeFromMotorList[i];
                // 获得故障属性(继续运行、减速停机、自由停机)
                errorAttributeTmp = GetErrorAttribute(motorErrorCodeTmp);
                // 优先级是否高于前一个故障
                if (errorAttributeTmp < errorAttribute)
                {
                    errorAttribute = errorAttributeTmp;
                    motorErrorCode = motorErrorCodeTmp;

                    // 获取故障信息    
                    motorErrorInfo = (motorErrInfor[i/4] >> ((i%4)*4)) & 0xF;
    ...
    // 获得性能故障
    errorCodeFromMotor = motorErrorCode;
    // 获得性能故障信息
    errorInfoFromMotor = motorErrorInfo;
#endif    
}
...
void GetNewError(void)
{
    ...
    if (devTcnt > (funcCode.code.devChkTime*(100/ERROR_DEAL_PERIOD)))
    {
     ...
     // 电机过速度
     // VF运行无效、转矩控制有效
    if (((ABS_INT32(frqFdbTmp) > ((Uint32)maxFrq * (1000 + funcCode.code.osChkValue)/1000))//当前速度不超过20.00hz时按最大频率20%
		 ||(ABS_INT32(frqFdbTmp) >(maxFrq + 1800)))              //当前速度不超过20.00hz时上限为18.00hz,不能在往上
        && (motorFc.motorCtrlMode == FUNCCODE_motorCtrlMode_FVC)  // 闭环控制
	...
	if (osTcnt > (funcCode.code.osChkTime*(1/ERROR_DEAL_PERIOD)))//单位1ms
    {
    ...
    if (errorCode == ERROR_COMM)
    {
        errorInfo = commErrInfo;
    }
...
#define ERROR_SCENE_NUMBER  9  // 故障现场记录个数
        ...
        funcCode.code.errorScene3.elem.errorCoeff = onlineCoeff;            // 故障时反电动势
        ...
        if ((ERROR_OC_ACC_SPEED == errorCodeFromMotor)   // 过流。// 性能传递故障
            ||(ERROR_BRAEK_SHORT== errorCode))          //制动电阻短路也按照过流故障处理
        {
            // 故障记录
...
void ErrorReset(void)
{
    ...
    //if (runFlag.bit.run)
    {
        ...
        if (++errAutoRstClrWaitTicker > TIME_UNIT_ERR_AUTO_RST_CLR / ERROR_DEAL_PERIOD)
        {
...
Uint16 AutoResetErrorTrueDeal(void)
{
    ...
    {
/*#ifndef DEBUG_MD500_SEARIS
		if(!(funcCode.code.LowVolErrorAutoRst))
...
Uint16 AutoResetErrorFcDeal(void)
{
    ...
    if ((ERROR_MOTOR_SHORT_TO_GND != errorCode)	 //  上电对地短路，不能复位
    	&&(ERROR_BRAEK_SHORT != errorCode)) 	 //  制动电阻短路
    {
...
Uint16 GetErrorAttribute(Uint16 errCode)
{
    ...
    else if(errCode == ERROR_COEFF)
    {
        level = ERROR_LEVEL_RUN;
    }
#if 0     
