...
// 5、电机参数的更新
//
// Time-stamp: <2008-03-14 14:08:23  Shisheng.Zhi, 0354>
//
//======================================================================
...
#endif
//#define SOFTWARE_VERSION     71          // 通用软件版本
//#define SOFTWARE_VERSION_BET  02          // 临时版本号
//#define SOFTWARE_VERSION_BET  03          // 临时版本号 方便canlink3.0连接测试 版本区别
#if DEBUG_MD500_SEARIS
// 定义通用软件版本信息
#define SOFTWARE_VERSION           1        // 通用软件版本
#define SOFTWARE_VERSION_TEMP      157         // 通用软件版本升级过程
...
// 定义临时版本软件信息
#define TEMP_SOFTWARE_NO           602        // 临时软件号
...
#elif DEBUG_MD290_SEARIS
// 定义通用软件版本信息
#define SOFTWARE_VERSION           3        // 通用软件版本
...
// 定义临时版本软件信息
#define TEMP_SOFTWARE_NO           100        // 临时软件号
#define TEMP_SOFTWARE_VERSION      0         // 临时软件升级过程
...
Uint16 coreSoftWareVesion;                          // 高8位:通用软件，低8位:升级过程
Uint16 coreNonStandardVesion;                       // 高10位:非标号，低6位:非标升级过程
Uint16 coreTmpsoftNoAndVersion;                     // 高10位:临时软件号，低6位:临时升级过程
Uint16 coreSoftType;                                // 高8位:通用或非标，低8位:是否临时版本

//--------------------------------//
/*
 * 关于软件版本号定义与现实
 * 通用软件定义: 71.01(71为当前通用版本号,01为修改升级过程)
 * 通用软件显示: F7-11 = v71.01(v开头显示表示当前为通用软件)
 * 非标软件定义: 308.00(308为非标号,00为升级过程)
 * 非标软件显示: F7-11 = 308.00(无v开头表示为非标软件)
 * 临时软件定义与显示:
 *   为非标软件临时版本:
 				 F7-11 = F308(F开头表示当前为临时版本,后面数据无小数点表示为非标版本上的临时版本)
 				 F7-08 = 1.00(1表示临时版本号, 00为临时版本的升级过程)
				 以上含义为308非标上的临时软件版本1,版本为1.00

 *   为标准软件临时版本:
                 F7-11 = F71.00(F开头表示当前为临时版本,后面数据有小数点表示为通用版本上的临时版本)   
                 F7-08 = 1.00(1表示临时版本号, 00为临时版本的升级过程)
				 以上含义为通用软件71.00上的临时软件版本1, 版本为1.00
 */
...
//               || (initTicker < START_DSP_DELAY_TIME_MS / 2)
           ...
           )
    {
        ...
        {
            case FUNCCODE_INIT_STATUS_VERIFY_EEPROM: // 判断EEPROM是第一次上电使用，EEPROM校验字1 和 EEPROM校验字2
            ...
            case FUNCCODE_INIT_STATUS_NEW_EEPROM:
                ...
                if (readAIAOChck == AIAO_CHK_CHECK)                            // 读取AIAO出厂校正值
                {
                    funcCodeRead.index = GetEepromIndexFromFcIndex(AI_AO_CHK_FLAG); // 读取AIAO校正字值
                    funcCodeRead.number = 1;
                    EepromDeal();
                    if (FUNCCODE_READ_RET_OK == ReadFuncCode(&funcCodeRead))
                    {
                        if (funcCodeRead.data[0] == AIAO_CHK_WORD)                  // 为0XA5表示已进行工装校正
                        {
                            readAIAOChck = AIAO_CHK_START;                          // 读取工装校正值
                            fcIndex = AI_AO_CALIB_START; // AIAO校正读取值地址
                        }
                        else
                        {
                            readAIAOChck = AIAO_CHK_END;                            // 直接开始初始化EEPROM
                        }
                    }
                }
                else if (readAIAOChck == AIAO_CHK_START)                            // 开始读取AIAO厂家参数
                {
                    static Uint16 flag = 0;
                    funcCodeRead.index = GetEepromIndexFromFcIndex(fcIndex);        // 根据功能码获取EEPROM地址
                    funcCodeRead.number = 1;
                    EepromDeal();
                    if (FUNCCODE_READ_RET_OK == ReadFuncCode(&funcCodeRead))
                    {
                        funcCode.all[fcIndex] = funcCodeRead.data[0];               // 存储功能码值
                        
                        fcIndex++;
                        flag++;

                        if (flag == 8)                                              // AI3不需要出厂校正
                        {
                            fcIndex += 4;
                        }

                        // 校正到AO1时结束
                        if (fcIndex > AI_AO_CALIB_STOP)
                        {
                            aiaoChckReadOK = AIAO_CHK_READ_OK;                     // AIAO出厂校验值读取正确
                ...
                {
                    funcCodeRwModeTmp = FUNCCODE_RW_MODE_WRITE_ALL;                // 参数恢复出厂值
                    EepromDeal();
                    if (!funcCodeRwModeTmp)                                        // 新EEPROM初始化完毕
                    {
                        funcCodeInitStatus = FUNCCODE_INIT_STATUS_OVER;
                        funcCode.code.eepromCheckWord1 = EEPROM_CHECK1;            // 存储新的EEPROM校验参数
                        funcCode.code.eepromCheckWord2 = EEPROM_CHECK2;
                        funcCode.code.aiaoChkWord = AIAO_CHK_WORD;                 // 存储AIAO厂家已校正参数
        ...
        //funcCode.code.paraInitMode = 0;
        funcCode.code.tuneCmd = 0;
        funcCode.code.factoryPassword = 0;
        funcCode.code.checkDigitalTube = 0; //上电确保不进入检测数码管显示
        PulseInSample();
...
// 判断EEPROM中的值是否在上下限之内
    if (FUNCCODE_INIT_STATUS_OVER == funcCodeInitStatus) // 读取功能码完毕
    {
        ...
        for (i = FNUM_PARA - 1; i >= 0; i--)    // 没有包括掉电保存和故障记录
        {
            ...
            {
                // 若上限受其他功能码限制琹imitedByOtherCodeIndex前面已经处理
                if (!pAttribute->attribute.bit.upperLimit)
                {
                    limitedByOtherCodeIndex[j++] = i;
                }
        ...
        lower = 5 * decNumber[funcCode.code.frqPoint];
...
// PLC掉电记忆
    ...
    curAbznumH = (signed long)funcCode.code.curABZPulseNumH<<16;
    curAbznumL = (signed long)funcCode.code.curABZPulseNumL;
    curABZPulseNum = curAbznumH + curAbznumL;
...
//=====================================================================
    ...
    dspMainCmd1.bit.shortGnd = funcCode.code.shortCheckMode%10;    // 上电对地短路保护功能(个位)
...
// PulseInSample()可以放在100us的循环中
	...
	#if DEBUG_MD500_SEARIS||DEBUG_MD290_SEARIS
    aiDeal[0].sample = gAI1;                          // 15   AI1的采样值，已经滤波
    aiDeal[1].sample = gAI2;                          // 16   AI2的采样值，已经滤波
    aiDeal[2].sample = gAI3;                          // 17   AI3的采样值，已经滤波
    #endif
...
//    SciDpDeal(&sciM380DpData);  //modefied by sjw 2009-12-24
    ...
    pulseInSamplePcQep();
#endif
    timer05ms.base = GetTime();   //当前基准时间   modfied by yz1990 2015-01-23
	if((timer05ms.baseOld - timer05ms.base + timer05ms.baseRemainder) >= 500*60)   //0.5ms
   	{
        commTicker++; //0.5ms++
		timer05ms.baseRemainder = timer05ms.baseOld - timer05ms.base + timer05ms.baseRemainder - 500*60;
		timer05ms.baseOld = timer05ms.base;  // 保存当前每个0.5ms开始计时时刻的计数器值
	}
}
...
void Main05msFunctionEachStart(void)
{
    ...
    //commTicker++;            // 串口的停顿时间 //不在该处计数，现改为在0ms里面计数500us为一个0.5ms modfied by yz1990 2015-01-23
...
void Main05msFunctionEachEnd(void)
{
    ...
    ticker05msFuncEach = timeBase - GetTime(); // 每个0.5ms执行间隔时间
...
void Main05msFunctionB(void)
{
    ...
    TorqueCalc();              // 更新设定转矩 80  防止切换模式时速度冲击，逻辑上保持一拍切换成功
...
Uint16 softVersionViewType;         // 显示类型
Uint16 softVersionVierPoint;        // 显示小数点
// 软件版本处理
void SoftVersionDeal(void)
{
    Uint16 softType, isTmpSoft;
    softType = SOFT_TYPE;           // 软件类型
    isTmpSoft = IS_TEMP_SOFTWARE;   // 是否为临时软件
    funcCode.code.productVersion = PRODUCT_VERSION;    // 产品号
    funcCode.code.funcSoftVersion = SOFTWARE_VERSION;  // 功能版本号

    if (isTmpSoft == 1) 
    {
        softVersionViewType = VIEW_TYPE_TEMP;
        funcCode.code.softVersion = TEMP_SOFTWARE_NO*100 + TEMP_SOFTWARE_VERSION;
    }
    else
    {
        if (softType == 1)
            softVersionViewType = VIEW_TYPE_NON_STANDAR;
        else
            softVersionViewType = VIEW_TYPE_STANDAR;
        funcCode.code.softVersion = 0;
    }

    // F7-11显示
    if (softType == 1)               // 非标
    {
        if (isTmpSoft == 1)          // 非标临时版本
        {
            funcCode.code.softVersion = NON_STANDARD_VERSION;   // 显示F+非标号
            softVersionVierPoint = 0;
        }
        else                         // 非标正式版本
        {
            funcCode.code.softVersion = NON_STANDARD_VERSION*100 + NON_STANDARD_VERSION_TEMP;  // 显示非标号+版本
            softVersionVierPoint = 2;
        }
    }
    else                             // 通用
    {
        funcCode.code.softVersion = SOFTWARE_VERSION*100 + SOFTWARE_VERSION_TEMP;
        softVersionVierPoint = 2;
    }
}
Uint16 coreSoftVersionViewType;         // 性能显示类型
Uint16 coreSoftVersionVierPoint;        // 性能显示小数点
//---------性能版版本显示控制
void coreSoftVersionDeal(void)
{
    Uint16 coreSoftTtpe,CoreIsTmpSoft;
	coreSoftWareVesion = (Uint16)(int16)coreSoftWareVesion;      // 高8位:通用软件，低8位:升级过程
	coreNonStandardVesion = (Uint16)(int16)coreNonStandardVesion;   // 高10位:非标号，低6位:非标升级过程
	coreTmpsoftNoAndVersion = (Uint16)(int16)coreTmpsoftNoAndVersion; // 高10位:临时软件号，低6位:临时升级过程 
	coreSoftType = (Uint16)(int16)coreSoftType;  	      // 高8位:通用或非标，低8位:是否临时版本 
	
    coreSoftTtpe = coreSoftType>>8;          // 性能类型
    CoreIsTmpSoft = coreSoftType & 0x00ff;  // 性能是否临时版本
    if (CoreIsTmpSoft == 1) 
    {
        coreSoftVersionViewType = VIEW_TYPE_TEMP;
        funcCode.code.softVersion = (coreTmpsoftNoAndVersion>>6)*100 + (coreTmpsoftNoAndVersion&0x003f);
    }
    else
    {
        if (coreSoftTtpe == 1)
            coreSoftVersionViewType = VIEW_TYPE_NON_STANDAR;
        else
            coreSoftVersionViewType = VIEW_TYPE_STANDAR;
        funcCode.code.softVersion = 0;
    }

    // F7-11显示
    if (coreSoftTtpe == 1)               // 非标
    {
        if (CoreIsTmpSoft == 1)          // 非标临时版本
        {
            funcCode.code.softVersion = coreNonStandardVesion>>6;   // 显示F+非标号
            coreSoftVersionVierPoint = 0;
        }
        else                         // 非标正式版本
        {
            funcCode.code.softVersion = (coreNonStandardVesion>>6)*100 + (coreNonStandardVesion&0x003f);  // 显示非标号+版本
            coreSoftVersionVierPoint = 2;
        }
    }
    else                             // 通用
    {
        funcCode.code.softVersion = (coreSoftWareVesion>>8)*100 + (coreSoftWareVesion&0x00ff);
        coreSoftVersionVierPoint = 2;
    }
}
...
#if 0
    // 判断U0组显示属性个数是否一致
    if (sizeof(dispAttributeD0) != U0NUM)
    {
        asm(" nop");
        asm(" ESTOP0");
    }
