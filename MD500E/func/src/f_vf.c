...
void vfSeparateDeal(void)
{
    ...
    if (vfSeparateVol < vfSeprateVolAim)    // 电压加速
    {
        volAccDecTime = (Uint32)funcCode.code.vfVoltageAccTime * (TIME_UNIT_VF_VOL_ACC_TIME / VF_CALC_PERIOD);
    }
    else                                    // 电压减速
    {
        volAccDecTime = (Uint32)funcCode.code.vfVoltageDecTime * (TIME_UNIT_VF_VOL_ACC_TIME / VF_CALC_PERIOD);
    }

     
// VF分离的电压上升时间
    vfSeprateVolAccLine.aimValue = vfSeprateVolAim;
    vfSeprateVolAccLine.tickerAll = volAccDecTime;
