...
int16 brakeUpperTorque;          // 速度控制时为制动转矩上限
int16 brakeTorqueAim;            // 速度控制时制动转矩目标设定
...
void UpdateTorqueAim(void)
{
	...
	Uint16 brakeTorqueSrc;   // 制动转矩源
    int16 brakeTorquePu;     // 制动转矩的PU
    ...
    if (RUN_MODE_TORQUE_CTRL != runMode)    
    {
		...
		// 速度控制(制动)转矩源、转矩上限
		brakeTorqueSrc = motorFc.vcPara.spdCtrlBrakeTorqueLimitSrc;
        brakeTorquePu = motorFc.vcPara.spdCtrlBrakeTorqueLimit;
    }
    // 转矩控制
    else    
    {
...
//                && (funcCode.code.slaveRevDataSel == MSC_RCV_TORQUE_SET)  // 从机数据作为转矩给定
                
                )
            { 
                tmp = ((int32)(int16)MasterSlaveCtrl.SlaveRcvTorque * 2000) / 10000;
            }
            else
#endif
            {
    ...
    brakeTorquePu *= TORQUE_UINT;
    // 制动转矩上限源
    switch (brakeTorqueSrc)
	{     
        case FUNCCODE_upperTorqueSrc_FC:  // 功能码设定
            tmp = brakeTorquePu;
            break;
        ...
        case FUNCCODE_upperTorqueSrc_AI2:  // AI2
        case FUNCCODE_upperTorqueSrc_AI3:  // AI3
            tmp = brakeTorqueSrc - FUNCCODE_upperTorqueSrc_AI1;
            tmp = ((int32)aiDeal[tmp].set * brakeTorquePu) >> 15;
            break;
    
        case FUNCCODE_upperTorqueSrc_PULSE:  // PULSE
            tmp = ((int32)pulseInSet * brakeTorquePu) >> 15;
            break;
...
//                && (funcCode.code.slaveRevDataSel == MSC_RCV_TORQUE_SET)  // 从机数据作为转矩给定
        ...
        case FUNCCODE_upperTorqueSrc_MIN_AI1_AI2:  // MIN(AI1,AI2)
            tmp = ((int32)aiDeal[0].set * brakeTorquePu) >> 15;
            tmp1 = ((int32)aiDeal[1].set * brakeTorquePu) >> 15;
...
void AccDecTorqueCalc(void)
{
    ...
    if (RUN_MODE_TORQUE_CTRL != runMode)    // 非转矩控制
    {
		...
		brakeUpperTorque = brakeTorqueAim;
        return;
    }
    ...
    if ((torqueAim == upperTorque)||(brakeTorqueAim == brakeUpperTorque))  // 已经达到目标转矩
    {
        return;
    }
...
#define RATING_TORQUE   (2000*TORQUE_UINT)    // 额定转矩, 100.0%
    // 直线加减速
	...
	brakeUpperTorque = upperTorque;//转矩模式，电动和发电转矩一样
