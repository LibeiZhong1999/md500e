...
//...恒转矩区域
    ...
    if(gVFPar.VFLineType == 1)	//多点VF曲线， 不用转矩提升
	{
...
void VFWSCompControl(void)
{
    ...
    if(( gMainCmd.FreqDesiredReal < 40  ) ||
        ( 16 < gInvInfo.InvTypeApply)) 	//设定频率小于0.4Hz,或者机型大?6
 	{	
        gWsComp.CompFreq = 0;
...
void VfOverCurDeal()
{
	...
	m_LimLow  = ((long)gOvCur.CurLim * 3605L)>>12;//4096*0.88
	m_LimHigh = ((long)gOvCur.CurLim * 3891L)>>12;//4096*0.95
...
/*void VfOverUdcDeal()
{
	...
	//else										m_Low = 6100;
	m_Low = 6100;//需要修改，此处的默认数值只是针对380V电压等级，其他电压等级?
