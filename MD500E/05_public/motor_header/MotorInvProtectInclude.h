...
#define  ERROR_INIT_POS			    (1L<< (15-1)) 	  // 15 初始位置故障-51

#if (SOFTSERIES == MD500SOFT)
#define  ERROR_OVERLOAD_BRAKE           (1L<< (16-1))     // 21 制动管过载 -61
#define  ERROR_IGBT_SHORT_BRAKE         (1L<< (17-1))     // 22 制动管直通 -62
#else
...
#define  ERROR_ENCODER              (1L<< (19-1))     // 19 信号编码器故障(20)
#define  ERROR_SHORT_BRAKE          (1L<< (20-1))     // 20 制动电阻直通 -60
#if (SOFTSERIES == MD380SOFT)
#define  ERROR_BRAKE_DETECT         (1L<< (21-1))    
#endif 
#define  ERROR_COEFF                (1L<< (22-1))     // 24 反电动势故障  -6４
...
#define UPSTATE    15000
#define DOWNSTATE  8500
#define EXSTATE    3000
/************************************************************/
/***********************结构体定义***************************/
...
typedef struct OVER_LOAD_PROTECT_DEF{
   ...
   Uint     OverLoadPreventEnable;
   Uint     GPOverLoadLine;   //GP过载曲线选择
   Uint     TotalPercent;   //过载程度
...
typedef struct BEFORE_RUN_PHASE_LOSE_STRUCT_DEF{
	Uint	Comper;
	Uint	OutPutLoseDetect;
	Uint	ShortGndDetect;
	Uint	ShortGndDetectFlag;		// 1- 将要进行或正在进行对地短路检测, 0-无需进行或已经结束对地短路检测
	Uint	CheckOverFlag;
	Uint	CheckFirstFlag;
	Uint	Counter;
	Uint	maxIU;
	Uint	maxIV;
	Uint	maxIW;
	Uint	OverFlag;					// 在对地短路检测中使用: 1- 有对地短路, 在起动前输出缺相检测中使用
	Uint	CurComperCoff;
	Uint    CurComperCoffLimit;
	Uint	UWPhaseLoseFlag;
	Uint	Flag;
    Uint    CurTotal;
}BEFORE_RUN_PHASE_LOSE_STRUCT; 	//输出缺相判断程序
...
typedef struct SOFT_INPUT_LOSE_STRUCT_DEF{
   Uint     MaxDetaU;
   Uint     uDCFilter; 
   Uint     uDC;
   Uint     A_uDC;
   ...
   long    	HalfInvPowerPU;
   Uint		InLoseAddTime;
   Uint		InLoseDeleteTime;
}SOFT_INPUT_LOSE_STRUCT; 	//软件输入缺相判断程序
#endif
...
// 采用脉冲方式的逐波限流变量定义    2011-10-21-chzq
   ...
   Uint     CBCIntFlag;     //CBC中断标志，1已产生中断
...
#if (SOFT_INPUT_DETECT == STLEN)
extern SOFT_INPUT_LOSE_STRUCT		gSoftInLose;
#endif
