...
typedef struct UV_AMP_COFF_STRUCT_DEF {
	Ulong	TotalU;
	Ulong	TotalV;
	Ulong	TotalVoltL;		// case3中电压之和, 0.1V
	Ulong	TotalIL;			// case3中电流之和, Q12
	Ulong	TotalVolt;			// case4中电压之和, 0.1V
    Ulong   TotalComper;		// case4中占空比之和
	Ulong   TotalComperL;		// case3中占空比之和
	Ulong   ComperTotal;
    Ulong   ComperTotalL;
	Ulong	TotalI;				// case4中电流之和, Q12
    Ulong	TotalIV;
	Uint	Number;				// 进入中断的次数, 在中断中累加, 占空比调节过程中清零
	Uint	Comper;				// EPWM1的比较值
	Uint	ComperL;
	Uint 	UDivVGet;
	Uint 	UDivV;
	Uint 	UDivVSave;
    ...
    Uint    Rs_PRD;             //定子调谐载频
	Uint    CurComperCoff;		// 目标电流, 目前辨识分为三段, 600 2048 4096
	Uint    CurComperCoffFilter;
	Uint    Temper;
	Uint    TemperV;            //V相电流
	Uint    Flag;				// 占空比变化的标志, 1-在减小
	Uint    BeforeRunTuneEnable; //启动前参数辨识使能
	Uint    OnlineTuneBemfVoltEnable;
	Uint    RsTune;
